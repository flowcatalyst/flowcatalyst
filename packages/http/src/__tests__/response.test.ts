...
import { Result, UseCaseError } from "@flowcatalyst/domain";
import {
	getErrorStatus,
	toErrorResponse,
	sendResult,
...
describe("Response Utilities", () => {
	...
	describe("toErrorResponse", () => {
		it("should create error response with details", () => {
			const error = UseCaseError.validation("INVALID_EMAIL", "Invalid email", {
				field: "email",
			});
		...
		it("should send failure with appropriate status", async () => {
			...
			app.get("/test", (c) => {
				const result = Result.failure(
					UseCaseError.notFound("NOT_FOUND", "User not found"),
				);
				return sendResult(c, result);
			});
	...
	describe("matchResult", () => {
		it("should call onSuccess for success result", async () => {
			...
			app.get("/test", (c) => {
				...
				return matchResult(c, result, (value) => c.json({ greeting: value }));
			});
	...
	describe("Response helpers", () => {
		...
		it("badRequest should return 400 with details", async () => {
			const app = new Hono();
			app.get("/test", (c) =>
				badRequest(c, "Invalid input", { field: "email" }),
			);
