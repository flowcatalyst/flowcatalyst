 ...
 *
 * This plugin integrates with TracingContext from @flowcatalyst/domain
 ...
 * ```
 */
const tracingPluginAsync: FastifyPluginAsync<TracingPluginOptions> = async (
	fastify,
	opts,
) => {
	const {
	...
	fastify.addHook("onRequest", async (request, reply) => {
		...
		const causationId =
			(request.headers[causationIdHeader] as string | undefined) ?? null;
 ...
 */
export function requireTracing(request: {
	tracing?: TracingData;
}): TracingData {
	const tracing = request.tracing;
	if (!tracing) {
		throw new Error(
			"Tracing context not available. Ensure tracingPlugin is registered.",
		);
	}
	return tracing;
}
