...
import { ExecutionContext } from "@flowcatalyst/domain";
...
const executionContextPluginAsync: FastifyPluginAsync = async (fastify) => {
	...
	fastify.decorateRequest("executionContext", {
		getter() {
			return (this as unknown as { _executionContext: ExecutionContext })
				._executionContext;
		},
		setter(value: ExecutionContext) {
			(
				this as unknown as { _executionContext: ExecutionContext }
...
export function requireExecutionContext(request: {
	...
	if (!ctx) {
		throw new Error(
			"ExecutionContext not available. Ensure executionContextPlugin is registered.",
		);
	}
	return ctx;
}
