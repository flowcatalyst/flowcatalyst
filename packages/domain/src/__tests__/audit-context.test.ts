...
describe("AuditContext", () => {
	describe("outside context", () => {
		...
		it("should throw from requirePrincipalId", () => {
			expect(() => AuditContext.requirePrincipalId()).toThrow(
				"Authentication required",
			);
		});

		it("should throw from requirePrincipal", () => {
			expect(() => AuditContext.requirePrincipal()).toThrow(
				"Authentication required",
			);
		});
	});
	...
	describe("runWithPrincipal", () => {
		const testPrincipal = AuditContext.createPrincipal(
			"principal-123",
			"USER",
			"CLIENT",
			"client-456",
		...
		it("should return function result", () => {
			const result = AuditContext.runWithPrincipal(
				testPrincipal,
				() => "result",
			);
	...
	describe("runWithPrincipalAsync", () => {
		...
		it("should work with async functions", async () => {
			const result = await AuditContext.runWithPrincipalAsync(
				testPrincipal,
				async () => {
	...
	describe("authorization checks", () => {
		describe("hasAccessToAllClients", () => {
			it("should return true for ANCHOR scope", () => {
				const anchor = AuditContext.createPrincipal(
					"p1",
					"USER",
					"ANCHOR",
					null,
			...
			it("should return false for CLIENT scope", () => {
				const client = AuditContext.createPrincipal(
					"p2",
					"USER",
					"CLIENT",
					"client-1",
			...
			it("should return false for PARTNER scope", () => {
				const partner = AuditContext.createPrincipal(
					"p3",
					"USER",
					"PARTNER",
					"client-1",
		...
		describe("hasRole", () => {
			it("should return true when principal has role", () => {
				const principal = AuditContext.createPrincipal(
					"p1",
					"USER",
					"CLIENT",
					"c1",
