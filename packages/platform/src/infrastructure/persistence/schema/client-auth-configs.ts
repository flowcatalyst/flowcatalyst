...
export const clientAuthConfigs = pgTable(
	"tnt_client_auth_configs",
	{
		...
		additionalClientIds: jsonb("additional_client_ids")
			.$type<string[]>()
			.notNull()
			.default([]),
		grantedClientIds: jsonb("granted_client_ids")
			.$type<string[]>()
			.notNull()
			.default([]),
	...
	(table) => [
		...
		index("tnt_client_auth_configs_primary_client_id_idx").on(
			table.primaryClientId,
		),
	],
);
