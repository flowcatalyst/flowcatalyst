...
export const platformConfigs = pgTable(
	...
	(table) => [
		...
		index("idx_app_platform_configs_app_section").on(
			table.applicationCode,
			table.section,
		),
	],
...
export const platformConfigAccess = pgTable(
	...
	(table) => [
		unique("uq_app_config_access_role").on(
			table.applicationCode,
			table.roleCode,
		),
...
export type PlatformConfigAccessRecord =
	typeof platformConfigAccess.$inferSelect;
export type NewPlatformConfigAccessRecord =
	typeof platformConfigAccess.$inferInsert;
