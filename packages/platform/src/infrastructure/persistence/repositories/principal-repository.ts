...
import { eq, asc, desc, and, or, sql, inArray, ilike } from "drizzle-orm";
...
} from "../schema/service-accounts.js";
...
export interface PrincipalRepository extends PaginatedRepository<Principal> {
	...
	findByServiceAccountCode(
		code: string,
		tx?: TransactionContext,
	): Promise<Principal | undefined>;
	/** Check if a service account code already exists. */
	existsByServiceAccountCode(
		code: string,
		tx?: TransactionContext,
	): Promise<boolean>;
...
export function createPrincipalRepository(
	 ...
	 */
	async function fetchApplicationAccess(
		principalId: string,
		tx?: TransactionContext,
	): Promise<string[]> {
	...
	return {
			...
			const saRecord = record.serviceAccountId
				? (
						await db(tx)
							.select()
							.from(serviceAccounts)
							.where(eq(serviceAccounts.id, record.serviceAccountId))
							.limit(1)
					)[0]
				: undefined;
		...
		async findByServiceAccountCode(
			code: string,
			tx?: TransactionContext,
		): Promise<Principal | undefined> {
		...
		async existsByServiceAccountCode(
			code: string,
			tx?: TransactionContext,
		): Promise<boolean> {
		...
		async findFilteredPaged(
			...
			if (filters.clientId)
				conditions.push(eq(principals.clientId, filters.clientId));
			if (filters.active !== undefined)
				conditions.push(eq(principals.active, filters.active));
			if (filters.email)
				conditions.push(eq(principals.email, filters.email.toLowerCase()));
			if (filters.search) {
				const pattern = `%${filters.search}%`;
				conditions.push(
					or(
						ilike(principals.name, pattern),
			...
			if (entity.type === "SERVICE" && entity.serviceAccount) {
				await db(tx)
					...
					.values({
						...
						whCredentialsCreatedAt:
							entity.serviceAccount.whCredentialsCreatedAt,
						whCredentialsRegeneratedAt:
							entity.serviceAccount.whCredentialsRegeneratedAt,
		...
		async update(
			...
			if (
				entity.accessibleApplicationIds.length > 0 ||
				entity.type === "USER"
			) {
				await this.setApplicationAccess(
					entity.id,
					[...entity.accessibleApplicationIds],
					tx,
				);
			}
		...
		async deleteById(id: string, tx?: TransactionContext): Promise<boolean> {
			...
			await db(tx)
				.delete(principalApplicationAccess)
				.where(eq(principalApplicationAccess.principalId, id));
		...
		async setApplicationAccess(
			...
			if (applicationIds.length > 0) {
				await db(tx)
					.insert(principalApplicationAccess)
					.values(
...
function recordToPrincipal(
	...
	saRecord?: ServiceAccountRecord | undefined,
 ...
 */
function recordToServiceAccountData(
	...
	return {
		...
		whSigningAlgorithm: (sa.whSigningAlgorithm ??
			"HMAC_SHA256") as SignatureAlgorithm,
