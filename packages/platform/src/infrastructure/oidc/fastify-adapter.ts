...
export function registerOAuthCompatibilityRoutes(
	...
	fastify.log.info(
		"OAuth compatibility routes registered (/oauth/* -> /oidc/*)",
	);
}
...
export function registerOidcEndpointRoutes(
	...
	fastify.register(async (instance) => {
		...
		instance.get<{ Params: { uid: string } }>(
			"/authorize/:uid",
			async (request, reply) => {
				const { uid } = request.params;
				const queryString = request.raw.url?.split("?")[1] ?? "";
				const oidcPath = queryString
					? `/authorize/${uid}?${queryString}`
					: `/authorize/${uid}`;
				await forwardToOidc(request, reply, oidcPath);
			},
		);
		...
		instance.get("/session/end/confirm", async (request, reply) => {
			const queryString = request.raw.url?.split("?")[1] ?? "";
			const oidcPath = queryString
				? `/session/end/confirm?${queryString}`
				: "/session/end/confirm";
			await forwardToOidc(request, reply, oidcPath);
		});
