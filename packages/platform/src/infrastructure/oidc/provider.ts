...
export interface OidcProviderConfig {
	...
	/** Optional repository for recording service account token attempts */
	loginAttemptRepository?: LoginAttemptRepository | undefined;
...
export function createOidcProvider(config: OidcProviderConfig): Provider {
	...
	const providerConfig: Configuration = {
		...
		extraTokenClaims: async (
			...
			if (tokenClientId) {
				const oauthClient =
					await oauthClientRepository.findByClientId(tokenClientId);
	...
	// Record service account token attempts using the OIDC provider's event system.
	// We only log client_credentials grants (service account OAuth tokens).
	if (config.loginAttemptRepository) {
		const repo = config.loginAttemptRepository;

		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		provider.on("grant.success", (ctx: any) => {
			const grantType = ctx.oidc?.params?.grant_type as string | undefined;
			if (grantType !== "client_credentials") return;

			const clientId = ctx.oidc?.client?.clientId as string | undefined;
			if (!clientId) return;

			// Look up the service account principal asynchronously
			oauthClientRepository
				.findByClientId(clientId)
				.then((oauthClient) => {
					const principalId = oauthClient?.serviceAccountPrincipalId ?? null;
					return repo.create({
						attemptType: "SERVICE_ACCOUNT_TOKEN",
						outcome: "SUCCESS",
						failureReason: null,
						identifier: clientId,
						principalId,
						ipAddress: (ctx.request?.ip as string | undefined) ?? null,
						userAgent:
							(ctx.request?.headers?.["user-agent"] as string | undefined) ??
							null,
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		provider.on("grant.error", (ctx: any, err: unknown) => {
			const grantType = ctx.oidc?.params?.grant_type as string | undefined;
			if (grantType !== "client_credentials") return;
			...
			// Determine failure reason from the error
			const errorCode =
			...
			const failureReason =
				errorCode === "invalid_client"
					? ("INVALID_CLIENT_SECRET" as const)
	...
	const ClientProto = (
		provider.Client as unknown as { prototype: Record<string, unknown> }
	).prototype;
