 ...
 */
export async function createJwtKeyService(
	config: JwtKeyServiceConfig,
): Promise<JwtKeyService> {
	const {
		issuer,
		privateKey,
		publicKey,
	...
	if (privateKey && publicKey) {
		...
		const result = await loadKeysFromEnvVars(
			privateKey,
			publicKey,
			previousPublicKey,
		);
		...
		const extractablePrivKey = await jose.importPKCS8(privatePem, "RS256", {
			extractable: true,
		});
...
async function loadOrBootstrapKeyDir(dir: string) {
	...
	const signingPrivateKey: JoseKey = await jose.importPKCS8(
		signingPair.privatePem,
		"RS256",
	);
	...
	const extractableKey = await jose.importPKCS8(
		signingPair.privatePem,
		"RS256",
		{
			extractable: true,
