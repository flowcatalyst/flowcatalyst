...
export async function createDispatchInfrastructure(
	...
	if (env.DISPATCH_QUEUE_TYPE === "SQS" && env.DISPATCH_QUEUE_URL) {
		const { createSqsPublisher } = await import(
		...
		logger.info(
			{ queueUrl: env.DISPATCH_QUEUE_URL },
			"SQS post-commit dispatch configured",
		);
	}
	...
	if (
		messagingEnabled &&
		env.DISPATCH_QUEUE_TYPE === "SQS" &&
		env.DISPATCH_QUEUE_URL
	) {
		...
		const { startDispatchScheduler } = await import(
			"../dispatch-scheduler/index.js"
		);
		dispatchSchedulerHandle = startDispatchScheduler({
			...
			config: {
				...
				staleQueuedThresholdMinutes:
					env.DISPATCH_SCHEDULER_STALE_THRESHOLD_MINUTES,
				staleQueuedPollIntervalMs:
					env.DISPATCH_SCHEDULER_STALE_POLL_INTERVAL_MS,
			},
		});
		logger.info(
			"Dispatch Scheduler started (messagingEnabled=true, SQS publisher)",
		);
	}

	// Resolve the platform application ID so audit logs can reference it.
	// This is a best-effort lookup — if missing (e.g. first boot before bootstrap),
	// audit logs will simply have a null applicationId.
	const platformApp = await repos.applicationRepository.findByCode("platform");
	const platformApplicationId = platformApp?.id ?? undefined;
