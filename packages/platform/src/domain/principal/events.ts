...
export interface UserLoggedInData {
	readonly userId: string;
	readonly email: string;
	readonly loginMethod: "INTERNAL" | "OIDC";
	readonly identityProviderCode: string | null;
	readonly flowcatalystClaims: {
		readonly email: string;
...
export class UserLoggedIn extends BaseDomainEvent<UserLoggedInData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(
		APP,
		DOMAIN,
		"user",
	...
	constructor(ctx: ExecutionContext, data: UserLoggedInData) {
		super(
			{
				eventType: UserLoggedIn.EVENT_TYPE,
				specVersion: UserLoggedIn.SPEC_VERSION,
				source: SOURCE,
				subject: DomainEvent.subject(APP, "user", data.userId),
				messageGroup: DomainEvent.messageGroup(APP, "user", data.userId),
			},
