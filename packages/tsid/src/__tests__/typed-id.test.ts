...
describe("deserialize", () => {
	...
	it("should throw for wrong prefix type", () => {
		...
		expect(() => deserialize("APPLICATION", external)).toThrow(
	...
	it("should throw for missing prefix", () => {
		...
		expect(() => deserialize("CLIENT", id)).toThrow(
			"expected prefix 'client_'",
		);
	});

	it("should throw for invalid TSID format", () => {
		expect(() => deserialize("CLIENT", "client_invalid")).toThrow(TypedIdError);
		expect(() => deserialize("CLIENT", "client_invalid")).toThrow(
			"Invalid TSID format",
		);
	});
});
