...
import { Result } from "@flowcatalyst/domain";
import {
	validateRequired,
	validateFormat,
	validateMaxLength,
...
describe("Validation", () => {
	...
	describe("validateRange", () => {
		...
		it("should pass for value at boundaries", () => {
			expect(Result.isSuccess(validateRange(1, 1, 100, "score", "ERR"))).toBe(
				true,
			);
			expect(Result.isSuccess(validateRange(100, 1, 100, "score", "ERR"))).toBe(
				true,
			);
		});
	...
	describe("validateOneOf", () => {
		...
		it("should fail for value not in list", () => {
			...
			if (Result.isFailure(result)) {
				...
				expect(result.error.details["allowedValues"]).toEqual([
					"active",
					"inactive",
				]);
			}
