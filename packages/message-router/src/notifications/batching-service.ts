...
export class BatchingNotificationService implements NotificationService {
	...
	constructor(
		...
		if (
			config.enabled &&
			(emailService?.isEnabled() || teamsService?.isEnabled())
		) {
			this.start();
		}
	}
		...
		this.logger.debug(
			{
				...
				pendingCount: this.pendingWarnings.length,
			},
	...
	async notifyCriticalError(error: WarningNotification): Promise<void> {
		...
		this.logger.info(
			...
			"Sending critical error notification immediately",
		);
	...
	async notifySystemEvent(event: SystemEventNotification): Promise<void> {
		...
		this.logger.debug(
			...
			"Sending system event notification",
		);
	...
	async flush(): Promise<void> {
		...
		this.logger.info({ count: warnings.length }, "Flushing batch notification");
	...
	private deduplicateWarnings(
		...
		for (const warning of warnings) {
			...
			if (!existing) {
				...
				(warning.severity === existing.severity &&
					warning.timestamp > existing.timestamp)
			) {
				seen.set(key, warning);
			}
		}
