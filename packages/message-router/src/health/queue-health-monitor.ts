 ...
 */
export type QueueStatsProvider = () =>
...
export class QueueHealthMonitor {
	...
	private checkQueueGrowth(queueName: string, stats: QueueStats): void {
		...
		if (growth >= this.config.growthThreshold) {
			...
			if (
				history.consecutiveGrowthPeriods >= this.config.growthPeriodsForWarning
			) {
				this.logger.warn(
					{
						queueName,
		...
		try {
			...
			for (const [queueName, stats] of entries) {
				...
				statuses.push({
					...
					isGrowing:
						consecutiveGrowthPeriods >= this.config.growthPeriodsForWarning,
