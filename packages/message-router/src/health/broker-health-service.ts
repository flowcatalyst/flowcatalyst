...
export class BrokerHealthService {
	...
	constructor(
		...
		this.logger.info(
			...
			"BrokerHealthService initialized",
		);
	}
	 ...
	 */
	checkBrokerConnectivity(): ResultAsync<
		BrokerHealthResult,
		BrokerHealthError
	> {
		if (!env.MESSAGE_ROUTER_ENABLED) {
			this.logger.debug(
				"Message router disabled, skipping broker connectivity check",
			);
	 ...
	 */
	checkSqsQueueAccessible(
		queueName: string,
	): ResultAsync<string, BrokerHealthError> {
		...
		return new Promise((resolve) => {
			...
			stompit.connect(connectOptions, (error, client) => {
				...
				if (error) {
					this.logger.error(
						{ err: error },
						"ActiveMQ connectivity check failed",
					);
					resolve(false);
					return;
				}
