...
import type { MonitoringHealthResponse } from "@flowcatalyst/contracts";
	...
	async getReadiness(): Promise<HealthCheckResult> {
		...
		brokerResult.match(
			(result) => {
				if (!result.healthy) {
					issues.push(
						result.details || `${result.broker} broker is not accessible`,
					);
				}
			},
			(error) => {
				issues.push(
					`Broker health check failed: ${error.type} - ${error.broker}`,
				);
			},
		);
