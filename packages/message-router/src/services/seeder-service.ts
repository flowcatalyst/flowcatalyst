...
export class SeederService {
	...
	async seedMessages(request: SeedRequest): Promise<SeedResult> {
		...
		this.logger.info(
			{ count, queue, endpoint, messageGroupMode },
			"Seeding messages",
		);
		...
		if (!this.queueManager.hasEmbeddedQueue()) {
			this.logger.warn(
				"Embedded queue not available - seeding only works in EMBEDDED mode",
			);
	...
	private getMessageGroupId(mode: string, index: number): string {
		switch (mode) {
			...
			default:
				return (
					this.messageGroups[index % this.messageGroups.length] ?? "group-a"
				);
		}
	}
