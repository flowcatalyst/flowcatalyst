...
/**
 * Common interface for fetching router configuration.
 * Implemented by both PlatformConfigClient (single source) and MultiConfigFetcher (multiple sources).
 */
export interface ConfigFetcher {
	fetchConfig(): Promise<MessageRouterConfig | null>;
	fetchConfigOnce(): Promise<MessageRouterConfig>;
}
	...
	private async doFetch(): Promise<MessageRouterConfig> {
		...
		try {
			...
			if (error instanceof Error && error.name === "AbortError") {
				throw new Error(
					`Request timeout after ${this.options.connectTimeoutMs + this.options.readTimeoutMs}ms`,
					{ cause: error },
				);
			}
			throw error;
...
export class ConfigSyncService {
	private readonly client: ConfigFetcher;
	private readonly logger: Logger;
