...
export const eventProjectionFeed = pgTable(
	...
	(table) => [
		...
		index('idx_event_projection_feed_unprocessed').on(table.id).where({ processed: 0 } as any),
		// Index for crash recovery
		index('idx_event_projection_feed_in_progress').on(table.id).where({ processed: 9 } as any),
	],
);
...
export const dispatchJobProjectionFeed = pgTable(
	...
	(table) => [
		...
		index('idx_dj_projection_feed_unprocessed').on(table.dispatchJobId, table.id).where({ processed: 0 } as any),
		// Index for crash recovery
		index('idx_dj_projection_feed_in_progress').on(table.id).where({ processed: 9 } as any),
		// Index for cleanup of old processed entries
		index('idx_dj_projection_feed_processed_at').on(table.processedAt).where({ processed: 1 } as any),
	],
);
