...
export class EmbeddedQueuePublisher {
  ...
  publish(message: EmbeddedQueueMessage): PublishResult {
    ...
    try {
      ...
      this.logger.error({ err: error, messageId: message.messageId }, 'Failed to publish message');
      return {
        messageId: message.messageId,
        success: false,
        error: errorMessage,
  ...
  private cleanupDeduplication(now: number): void {
    ...
    try {
      this.db.run('DELETE FROM message_deduplication WHERE created_at < ?', [cutoff]);
    } catch (error) {
