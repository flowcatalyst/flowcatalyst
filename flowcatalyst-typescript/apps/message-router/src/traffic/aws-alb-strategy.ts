    ...
    const checkHealth = (): ResultAsync<void, TrafficError> => {
      ...
      return ResultAsync.fromPromise(
        ...
        return ResultAsync.fromPromise(
          ...
          () => TrafficErrors.deregistrationFailed(this.getName(), new Error('Wait interrupted')),
        ).andThen(() => checkHealth());
      });
    };
 ...
 */
export function createAwsAlbStrategy(config: AwsAlbStrategyConfig, logger: Logger): AwsAlbStrategy {
