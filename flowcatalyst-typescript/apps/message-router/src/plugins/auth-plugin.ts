...
function handleOidcAuth(
	...
	return async (request, reply) => {
		...
		if (!authHeader) {
			...
			reply
				.code(401)
				.header('WWW-Authenticate', 'Bearer realm="FlowCatalyst Message Router", error="missing_token"')
		...
		if (!authHeader.startsWith('Bearer ')) {
			...
			reply.code(400).send({ error: 'Invalid authorization format, expected Bearer token' });
			return;
		}
		...
		try {
			...
			if (config.clientId) {
				...
				if (!hasValidClientId) {
					childLogger.warn({ azp, aud, expectedClientId: config.clientId }, 'Token client ID mismatch');
					reply
						.code(401)
						.header(
							'WWW-Authenticate',
...
const authPluginAsync: FastifyPluginAsync<AuthPluginOptions> = async (fastify, opts) => {
	...
	fastify.addHook('onRequest', async (request, reply) => {
		...
		if (!request.url.startsWith('/api/') && !request.url.startsWith('/monitoring/')) {
			return;
		}
