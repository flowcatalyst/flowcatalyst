...
export class HealthService {
  ...
  getSystemHealth(): MonitoringHealthResponse {
    ...
    const healthyQueues = Object.values(queueStats).filter((s) => s.successRate30min >= 0.9).length;

    const totalPools = Object.keys(poolStats).length;
    const healthyPools = Object.values(poolStats).filter((s) => s.successRate30min >= 0.9).length;
