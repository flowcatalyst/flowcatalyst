...
export class QueueManagerService {
  ...
  private async monitorAndRestartUnhealthyConsumers(): Promise<void> {
    ...
    const consumerCount =
      this.consumers.size + this.activeMqConsumers.size + this.natsConsumers.size;
  ...
  private async restartSqsConsumer(queueUri: string, consumer: SqsConsumer): Promise<void> {
    ...
    try {
      ...
      const newConsumer = this.createSqsConsumer(
        queueUri,
   ...
   */
  private async restartActiveMqConsumer(
    queueName: string,
  ...
  private async initializeEmbeddedMode(): Promise<void> {
    ...
    await this.embeddedQueue.startConsumer(async (batch, callbacks) =>
      this.handleEmbeddedBatch(batch, callbacks),
    );
