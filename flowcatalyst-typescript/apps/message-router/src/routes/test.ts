...
export const testRoutes: FastifyPluginAsync = async (fastify) => {
	...
	fastify.post('/faulty', {
		...
		if (random < 0.6) {
			...
			return reply.code(400).send({
				status: 'error', endpoint: 'faulty', requestId, messageId, error: 'Bad Request',
			});
		} else {
			return reply.code(500).send({
				status: 'error', endpoint: 'faulty', requestId, messageId, error: 'Internal Server Error',
			});
		}
	});
	...
	fastify.post('/fail', {
		...
		const requestId = ++requestCounter;
		return reply.code(500).send({
			status: 'error', endpoint: 'fail', requestId, error: 'Always fails',
		});
	});

	// POST /api/test/success
	fastify.post('/success', {
		schema: {
			tags: ['Test'],
	...
	fastify.post('/client-error', {
		...
		const requestId = ++requestCounter;
		return reply.code(400).send({
			status: 'error', endpoint: 'client-error', requestId, error: 'Record not found',
		});
	});

	// POST /api/test/server-error
	fastify.post('/server-error', {
		schema: {
			tags: ['Test'],
		...
		const requestId = ++requestCounter;
		return reply.code(500).send({
			status: 'error', endpoint: 'server-error', requestId,
		});
	});
