...
export class ActiveMqConsumer implements QueueConsumer {
    ...
    this.channelPool.channel((error: Error | null, channel: stompit.Channel, done: () => void) => {
      ...
      const subscription = channel.subscribe(
        ...
        (
          ...
          rawSubscription: StompitSubscription,
        ) => {
          if (subscribeError) {
          ...
          message.on('end', () => {
            this.handleMessage(channel, message, body, consumerId).catch((err) => {
  ...
  recreate(handler: StandardBatchHandler): ActiveMqConsumer {
    return new ActiveMqConsumer(this.config, handler, this.logger, this.instanceId);
  }
