...
export class SqsConsumer implements QueueConsumer {
  ...
  async start(): Promise<void> {
    ...
    this.running = true;
    this.logger.info({ connections: this.config.connections }, 'Starting SQS consumer');
  ...
  getHealth(): ConsumerHealth {
    ...
    const timeSinceLastPollMs = this.lastPollTimeMs > 0 ? currentTimeMs - this.lastPollTimeMs : -1;
    ...
    for (const sqsMsg of sqsMessages) {
      ...
      try {
        ...
        pointer = {
          ...
          messageGroupId:
            parsed.messageGroupId || sqsMsg.Attributes?.MessageGroupId || '__DEFAULT__',
      ...
      if (seenMessageIds.has(pointer.messageId)) {
        this.logger.debug({ messageId: pointer.messageId }, 'Duplicate message in batch - ACKing');
        await this.deleteMessage(sqsMsg.ReceiptHandle);
        continue;
      }
      ...
      const queueMessage: QueueMessage = {
        ...
        pointer,
        receiveCount: Number.parseInt(sqsMsg.Attributes?.ApproximateReceiveCount || '1', 10),
        receivedAt,
        batchId,
   ...
   */
  private async nackMessage(receiptHandle: string, visibilityTimeoutSeconds = 30): Promise<void> {
    try {
  ...
  private async metricsLoop(): Promise<void> {
    ...
    while (this.running) {
      try {
        ...
        this.pendingMessages = Number.parseInt(attrs['ApproximateNumberOfMessages'] || '0', 10);
