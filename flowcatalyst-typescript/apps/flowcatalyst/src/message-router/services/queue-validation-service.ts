...
export class QueueValidationService {
	...
	async validateQueues(
		queueConfigs: QueueConfigForValidation[],
		...
		if (issues.length === 0) {
			this.logger.info(
				...
				"All queues validated successfully",
			);
		} else {
			this.logger.warn(
	 ...
	 */
	private async validateSqsQueue(
		config: QueueConfigForValidation,
	): Promise<boolean> {
		...
		try {
			...
			if (queueUrl && queueUrl.trim() !== "") {
				...
				if (!queueUrl.startsWith("http")) {
					this.logger.warn(
						{ queueUrl },
						"Queue URI doesn't appear to be a valid URL",
					);
					return false;
				}
			...
			this.logger.error(
				{ err: error, queueName: config.queueName },
				"Failed to validate SQS queue",
			);
			return false;
		} finally {
			sqsClient.destroy();
	 ...
	 */
	private async validateActiveMqQueue(
		config: QueueConfigForValidation,
	): Promise<boolean> {
		return new Promise((resolve) => {
			...
			stompit.connect(connectOptions, (error, client) => {
				...
				this.logger.debug(
					{ queueName },
					"ActiveMQ queue validated successfully",
				);
				client.disconnect();
				resolve(true);
			});
