...
export class EmbeddedQueueConsumer implements QueueConsumer {
  ...
  private ack(receiptHandle: string): void {
    try {
      this.db.run('DELETE FROM queue_messages WHERE receipt_handle = ?', [receiptHandle]);
  ...
  private nack(receiptHandle: string, visibilityTimeoutSeconds?: number): void {
    ...
    try {
      this.db.run('UPDATE queue_messages SET visible_at = ? WHERE receipt_handle = ?', [
        visibleAt,
        receiptHandle,
      ]);
  ...
  recreate(handler: StandardBatchHandler): EmbeddedQueueConsumer {
    return new EmbeddedQueueConsumer(this.db, this.config, handler, this.logger, this.instanceId);
  }
}
