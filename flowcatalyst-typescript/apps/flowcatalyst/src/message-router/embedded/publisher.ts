...
export class EmbeddedQueuePublisher {
	...
	publish(message: EmbeddedQueueMessage): PublishResult {
		...
		try {
			...
			if (message.messageDeduplicationId) {
				const isDuplicate = this.checkDeduplication(
			...
			if (
				errorMessage.includes(
					"UNIQUE constraint failed: queue_messages.message_id",
				)
			) {
				this.logger.debug(
					{ messageId: message.messageId },
	...
	private cleanupDeduplication(now: number): void {
		...
		try {
			this.db.run("DELETE FROM message_deduplication WHERE created_at < ?", [
				cutoff,
			]);
		} catch (error) {
			this.logger.warn(
				{ err: error },
				"Failed to cleanup deduplication entries",
			);
		}
	}
