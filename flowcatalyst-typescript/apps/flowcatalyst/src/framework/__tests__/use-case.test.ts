...
describe("UseCase Pattern", () => {
	...
	class CreateUserUseCase
		implements UseCase<CreateUserCommand, UserCreatedEvent>
	{
		constructor(
		...
		async execute(
			...
			if (Result.isFailure(nameResult))
				return nameResult as Result<UserCreatedEvent>;

			const emailResult = validateEmail(command.email);
			if (Result.isFailure(emailResult))
				return emailResult as Result<UserCreatedEvent>;
	...
	it("should pass command to UnitOfWork for audit", async () => {
		...
		const mockUnitOfWork: UnitOfWork = {
			commit: async <T extends DomainEvent>(
				_agg: Aggregate,
				event: T,
				command: unknown,
			) => {
