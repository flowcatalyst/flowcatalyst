...
async function resolveFrontendDir(): Promise<string | undefined> {
	...
	try {
		...
		if (sea.isSea()) {
			...
			const data = JSON.parse(raw) as { files: Record<string, { content: string; encoding: 'utf8' | 'base64' }> };
...
async function runRotateKeysCommand(): Promise<void> {
	...
	const { generateKeyPair, writeKeyPair, loadKeyDir, removeKeyPair } = await import('@flowcatalyst/platform');
...
const AUTO_MIGRATE = process.env['AUTO_MIGRATE'] !== undefined
...
async function main() {
	...
	if (enabledServices.length === 0) {
		logger.warn('No services enabled. Set PLATFORM_ENABLED, STREAM_PROCESSOR_ENABLED, or MESSAGE_ROUTER_ENABLED to true.');
		process.exit(1);
	}
