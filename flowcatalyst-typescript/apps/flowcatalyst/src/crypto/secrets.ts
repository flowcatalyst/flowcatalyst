...
export class EncryptedSecretProvider implements SecretProvider {
	...
	async prepareForStorage(
		reference: string,
	): Promise<Result<string, SecretError>> {
		if (this.isPlaintextReference(reference)) {
			const plaintext = reference.slice(
				EncryptedSecretProvider.PLAINTEXT_PREFIX.length,
			);
	...
	async resolve(reference: string): Promise<Result<string, SecretError>> {
		if (!this.enabled) {
			return err({
				...
				message:
					"AWS Secrets Manager is not enabled. Set FLOWCATALYST_AWS_SM_ENABLED=true",
			});
		}
	...
	async resolve(reference: string): Promise<Result<string, SecretError>> {
		if (!this.enabled) {
			return err({
				...
				message:
					"AWS Parameter Store is not enabled. Set FLOWCATALYST_AWS_PS_ENABLED=true",
			});
		}
...
export class VaultSecretProvider implements SecretProvider {
	...
	constructor(
		...
		this.address =
			config.address ?? process.env["VAULT_ADDR"] ?? "http://127.0.0.1:8200";
	...
	async resolve(reference: string): Promise<Result<string, SecretError>> {
		if (!this.enabled) {
			return err({
				...
				provider: "vault",
				message:
					"HashiCorp Vault is not enabled. Set FLOWCATALYST_VAULT_ENABLED=true",
			});
		}
...
export class SecretService {
	...
	): Promise<Result<string | null, SecretError>> {
 ...
 */
export function createSecretServiceFromEnv(
