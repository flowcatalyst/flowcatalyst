...
export interface PlatformConfigServiceDeps {
	readonly configRepository: {
		...
		insert(
			entity: ReturnType<typeof createPlatformConfig>,
		): Promise<PlatformConfig>;
...
export function createPlatformConfigService(
	deps: PlatformConfigServiceDeps,
	...
	return {
		...
		async deleteValue(
			...
			return configRepository.deleteByKey(
				applicationCode,
				section,
				property,
				scope,
		...
		async getConfigs(
			...
			return configRepository.findByApplication(
				applicationCode,
				scope,
				clientId,
			);
		...
		async canAccess(
			...
			const grants = await accessRepository.findByRoleCodes(
				applicationCode,
				userRoles,
			);
