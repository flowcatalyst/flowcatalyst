 ...
 */
export type NewClientAuthConfig = Omit<
	ClientAuthConfig,
	"createdAt" | "updatedAt"
> & {
	createdAt?: Date;
 ...
 */
export function createInternalAuthConfig(
	input: CreateInternalAuthConfigInput,
): NewClientAuthConfig {
	return {
...
export function createOidcAuthConfig(
	input: CreateOidcAuthConfigInput,
): NewClientAuthConfig {
	return {
		id: generate("CLIENT_AUTH_CONFIG"),
		emailDomain: input.emailDomain.toLowerCase(),
 ...
 */
export function validateConfigTypeConstraints(
 ...
 */
export function getAllAccessibleClientIds(
	config: ClientAuthConfig,
): readonly string[] {
	switch (config.configType) {
		case "ANCHOR":
