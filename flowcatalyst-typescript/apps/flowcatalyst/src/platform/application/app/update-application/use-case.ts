    ...
    async execute(
      ...
      if (!application) {
        return Result.failure(
          UseCaseError.notFound('APPLICATION_NOT_FOUND', 'Application not found'),
        );
      }
      ...
      const updatedApplication = updateApplication(application, {
        name: command.name,
        description:
          command.description !== undefined ? command.description : application.description,
        ...
        logoMimeType:
          command.logoMimeType !== undefined ? command.logoMimeType : application.logoMimeType,
        defaultBaseUrl:
