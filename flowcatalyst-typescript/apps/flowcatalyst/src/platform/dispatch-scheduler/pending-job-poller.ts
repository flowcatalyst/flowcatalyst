...
import { dispatchJobs, type DispatchJobRecord, type DispatchMode } from '@flowcatalyst/persistence';
...
export function createPendingJobPoller(
  ...
  async function doPoll(): Promise<void> {
    ...
    try {
      ...
      logger.debug({ count: pendingJobs.length }, 'Found pending jobs to process');
      ...
      const blockedGroups = await blockOnErrorChecker.getBlockedGroups(groupKeys);
  ...
  return {
    start() {
      ...
      logger.info({ pollIntervalMs: config.pollIntervalMs }, 'PendingJobPoller started');
    },
