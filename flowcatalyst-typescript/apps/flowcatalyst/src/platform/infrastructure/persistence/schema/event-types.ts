...
export const eventTypes = pgTable(
	...
	(table) => [
		index("idx_msg_event_types_code").on(table.code),
		index("idx_msg_event_types_status").on(table.status),
		index("idx_msg_event_types_source").on(table.source),
		index("idx_msg_event_types_application").on(table.application),
		index("idx_msg_event_types_subdomain").on(table.subdomain),
		index("idx_msg_event_types_aggregate").on(table.aggregate),
	],
);
...
export const eventTypeSpecVersions = pgTable(
	...
	(table) => [
		index("idx_msg_spec_versions_event_type").on(table.eventTypeId),
		index("idx_msg_spec_versions_status").on(table.status),
		unique("uq_msg_spec_versions_event_type_version").on(
			table.eventTypeId,
			table.version,
		),
	],
...
export type EventTypeSpecVersionRecord =
	typeof eventTypeSpecVersions.$inferSelect;
export type NewEventTypeSpecVersionRecord =
	typeof eventTypeSpecVersions.$inferInsert;
