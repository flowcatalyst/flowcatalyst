...
export type NewEmailDomainMappingRecord =
	typeof emailDomainMappings.$inferInsert;
...
export const emailDomainMappingAdditionalClients = pgTable(
	"tnt_email_domain_mapping_additional_clients",
	{
		...
		emailDomainMappingId: varchar("email_domain_mapping_id", {
			length: 17,
		}).notNull(),
		clientId: varchar("client_id", { length: 17 }).notNull(),
	},
	(table) => [
		index("idx_tnt_edm_additional_clients_mapping").on(
			table.emailDomainMappingId,
		),
	],
);

export type EmailDomainMappingAdditionalClientRecord =
	typeof emailDomainMappingAdditionalClients.$inferSelect;
...
export const emailDomainMappingGrantedClients = pgTable(
	...
	(table) => [
		index("idx_tnt_edm_granted_clients_mapping").on(table.emailDomainMappingId),
	],
);

export type EmailDomainMappingGrantedClientRecord =
	typeof emailDomainMappingGrantedClients.$inferSelect;
...
export const emailDomainMappingAllowedRoles = pgTable(
	...
	(table) => [
		index("idx_tnt_edm_allowed_roles_mapping").on(table.emailDomainMappingId),
	],
);

export type EmailDomainMappingAllowedRoleRecord =
	typeof emailDomainMappingAllowedRoles.$inferSelect;
