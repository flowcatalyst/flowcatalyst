 ...
 */
export function createClientAccessGrantRepository(
	defaultDb: AnyDb,
	...
	return {
		...
		async deleteByPrincipalAndClient(
			...
			const exists = await this.existsByPrincipalAndClient(
				principalId,
				clientId,
				tx,
			);
		...
		async delete(
			entity: ClientAccessGrant,
			tx?: TransactionContext,
		): Promise<boolean> {
			return this.deleteById(entity.id, tx);
		},

		async findByPrincipalIds(
			principalIds: string[],
			tx?: TransactionContext,
		): Promise<Map<string, string[]>> {
			...
			const records = await db(tx)
				.select({
					principalId: clientAccessGrants.principalId,
					clientId: clientAccessGrants.clientId,
				})
				.from(clientAccessGrants)
 ...
 */
function recordToClientAccessGrant(
	record: ClientAccessGrantRecord,
): ClientAccessGrant {
	return {
		id: record.id,
