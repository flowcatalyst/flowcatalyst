...
export function createPrincipalRepository(defaultDb: AnyDb): PrincipalRepository {
  ...
  return {
    async findById(id: string, tx?: TransactionContext): Promise<Principal | undefined> {
      const [record] = await db(tx).select().from(principals).where(eq(principals.id, id)).limit(1);
    ...
    async findByType(type: PrincipalType, tx?: TransactionContext): Promise<Principal[]> {
      const records = await db(tx).select().from(principals).where(eq(principals.type, type));
...
function recordToServiceAccountData(
  sa: import('../schema/service-accounts.js').ServiceAccountRecord,
): ServiceAccountData {
  return {
    code: sa.code,
