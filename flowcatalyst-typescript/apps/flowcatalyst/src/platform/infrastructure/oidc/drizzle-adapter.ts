  ...
  return class DrizzleAdapter implements Adapter {
    ...
    async findByUserCode(userCode: string): Promise<AdapterPayload | undefined> {
      const [record] = await db
        .select()
        .from(oidcPayloads)
        .where(and(eq(oidcPayloads.type, this._name), eq(oidcPayloads.userCode, userCode)))
        .limit(1);
    ...
    async findByUid(uid: string): Promise<AdapterPayload | undefined> {
      const [record] = await db
        .select()
        .from(oidcPayloads)
        .where(and(eq(oidcPayloads.type, this._name), eq(oidcPayloads.uid, uid)))
        .limit(1);
    ...
    async consume(id: string): Promise<void> {
      ...
      await db.update(oidcPayloads).set({ consumedAt: new Date() }).where(eq(oidcPayloads.id, key));
    }
 ...
 */
export async function cleanupExpiredPayloads(db: PostgresJsDatabase): Promise<number> {
