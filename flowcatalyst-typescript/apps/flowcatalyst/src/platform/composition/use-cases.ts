...
export function createUseCases(deps: CreateUseCasesDeps) {
  ...
  const updateClientUseCase = createGuardedUseCase(
    createUpdateClientUseCase({ clientRepository: repos.clientRepository, unitOfWork }),
    clientAccessGuard((cmd) => cmd.clientId),
  );

  const changeClientStatusUseCase = createGuardedUseCase(
    createChangeClientStatusUseCase({ clientRepository: repos.clientRepository, unitOfWork }),
    clientAccessGuard((cmd) => cmd.clientId),
  );

  const deleteClientUseCase = createGuardedUseCase(
    createDeleteClientUseCase({ clientRepository: repos.clientRepository, unitOfWork }),
    clientAccessGuard((cmd) => cmd.clientId),
  );

  const addClientNoteUseCase = createGuardedUseCase(
    createAddClientNoteUseCase({ clientRepository: repos.clientRepository, unitOfWork }),
    clientAccessGuard((cmd) => cmd.clientId),
  );
