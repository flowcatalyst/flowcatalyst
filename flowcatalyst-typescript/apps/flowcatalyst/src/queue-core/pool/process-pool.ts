...
import type { PoolConfig, PoolState, PoolStats, QueueMessage } from '@flowcatalyst/shared-types';
...
export class ProcessPool {
   ...
   */
  async submit(message: QueueMessage, callback: MessageCallback): Promise<boolean> {
   ...
   */
  private async processMessage(message: QueueMessage, callback: MessageCallback): Promise<void> {
    const batchGroupKey = `${message.batchId}|${message.pointer.messageGroupId}`;
  ...
  getStats(): PoolStats {
    ...
    const successRate = this.totalProcessed > 0 ? this.totalSucceeded / this.totalProcessed : 1.0;
    ...
    return {
      ...
      successRate5min:
        this.stats5min.processed > 0 ? this.stats5min.succeeded / this.stats5min.processed : 1.0,
      ...
      successRate30min:
        this.stats30min.processed > 0 ? this.stats30min.succeeded / this.stats30min.processed : 1.0,
  ...
  updateConfig(newConfig: Partial<PoolConfig>): void {
    ...
    if (newConfig.concurrency !== undefined) {
      ...
      this.logger.info({ concurrency: newConfig.concurrency }, 'Concurrency updated in-place');
    }
  }
