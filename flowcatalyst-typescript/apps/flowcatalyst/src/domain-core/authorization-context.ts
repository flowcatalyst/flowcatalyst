...
export const AuthorizationContext = {
   ...
   */
  canAccessResourceWithPrefix(authz: AuthorizationContext, resourceCode: string): boolean {
  ...
  canAccessClient(authz: AuthorizationContext, clientId: string): boolean {
    ...
    return authz.accessibleClientIds !== null && authz.accessibleClientIds.has(clientId);
  },
  ...
  isPlatformAdmin(authz: AuthorizationContext): boolean {
    return authz.roles.has('platform:super-admin') || authz.roles.has('platform:platform-admin');
  },
