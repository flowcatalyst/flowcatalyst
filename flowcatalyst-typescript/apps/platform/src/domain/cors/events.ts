...
export class CorsOriginAdded extends BaseDomainEvent<CorsOriginAddedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'cors-origin', 'added');
		...
		super(
			{
				...
				messageGroup: DomainEvent.messageGroup(APP, 'cors-origin', data.originId),
			},
			ctx,
			data,
		);
...
export class CorsOriginDeleted extends BaseDomainEvent<CorsOriginDeletedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'cors-origin', 'deleted');
	static readonly SPEC_VERSION = '1.0';
