...
export class EmailDomainMappingCreated extends BaseDomainEvent<EmailDomainMappingCreatedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'email-domain-mapping', 'created');
		...
		super(
			{
				...
				source: SOURCE,
				subject: DomainEvent.subject(APP, 'email-domain-mapping', data.emailDomainMappingId),
				messageGroup: DomainEvent.messageGroup(APP, 'email-domain-mapping', data.emailDomainMappingId),
			},
			ctx,
			data,
		);
...
export class EmailDomainMappingUpdated extends BaseDomainEvent<EmailDomainMappingUpdatedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'email-domain-mapping', 'updated');
	static readonly SPEC_VERSION = '1.0';
...
export class EmailDomainMappingDeleted extends BaseDomainEvent<EmailDomainMappingDeletedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'email-domain-mapping', 'deleted');
	static readonly SPEC_VERSION = '1.0';
