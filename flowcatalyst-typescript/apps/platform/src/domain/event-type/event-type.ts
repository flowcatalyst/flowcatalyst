...
import type { SpecVersion, NewSpecVersion } from './spec-version.js';
 ...
 */
export function buildCode(application: string, subdomain: string, aggregate: string, event: string): string {
...
export function createEventType(params: {
	...
	return {
		...
		code: buildCode(params.application, params.subdomain, params.aggregate, params.event),
		name: params.name,
 ...
 */
export function addSpecVersion(eventType: EventType, specVersion: SpecVersion): EventType {
	return {
		...eventType,
...
export function updateSpecVersion(
	...
	return {
		...eventType,
		specVersions: eventType.specVersions.map((sv) => (sv.version === version ? updater(sv) : sv)),
		updatedAt: new Date(),
	};
}
 ...
 */
export function findSpecVersion(eventType: EventType, version: string): SpecVersion | undefined {
...
export function allVersionsDeprecated(eventType: EventType): boolean {
	return eventType.specVersions.length > 0 && eventType.specVersions.every((sv) => sv.status === 'DEPRECATED');
}
