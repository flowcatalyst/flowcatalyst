...
export class ServiceAccountCreated extends BaseDomainEvent<ServiceAccountCreatedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'service-account', 'created');
		...
		super(
			{
				...
				source: SOURCE,
				subject: DomainEvent.subject(APP, 'service-account', data.serviceAccountId),
				messageGroup: DomainEvent.messageGroup(APP, 'service-account', data.serviceAccountId),
			},
			ctx,
			data,
		);
...
export class ServiceAccountUpdated extends BaseDomainEvent<ServiceAccountUpdatedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'service-account', 'updated');
	static readonly SPEC_VERSION = '1.0';
...
export class AuthTokenRegenerated extends BaseDomainEvent<AuthTokenRegeneratedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'service-account', 'auth-token-regenerated');
	static readonly SPEC_VERSION = '1.0';
...
export class SigningSecretRegenerated extends BaseDomainEvent<SigningSecretRegeneratedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'service-account', 'signing-secret-regenerated');
	static readonly SPEC_VERSION = '1.0';
...
export class ServiceAccountDeleted extends BaseDomainEvent<ServiceAccountDeletedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'service-account', 'deleted');
	static readonly SPEC_VERSION = '1.0';
