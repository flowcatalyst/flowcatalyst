...
export class IdentityProviderCreated extends BaseDomainEvent<IdentityProviderCreatedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'identity-provider', 'created');
		...
		super(
			{
				...
				source: SOURCE,
				subject: DomainEvent.subject(APP, 'identity-provider', data.identityProviderId),
				messageGroup: DomainEvent.messageGroup(APP, 'identity-provider', data.identityProviderId),
			},
			ctx,
			data,
		);
...
export class IdentityProviderUpdated extends BaseDomainEvent<IdentityProviderUpdatedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'identity-provider', 'updated');
	static readonly SPEC_VERSION = '1.0';
...
export class IdentityProviderDeleted extends BaseDomainEvent<IdentityProviderDeletedData> {
	static readonly EVENT_TYPE = DomainEvent.eventType(APP, DOMAIN, 'identity-provider', 'deleted');
	static readonly SPEC_VERSION = '1.0';
