...
import { createSubscription, SubscriptionCreated, type SubscriptionSource } from '../../../domain/index.js';
...
export function createCreateSubscriptionUseCase(
	...
	return {
		async execute(
			...
			if (codeExists) {
				return Result.failure(
					UseCaseError.businessRule('CODE_EXISTS', 'Subscription code already exists in this scope', {
						code: command.code,
					}),
				);
			}
