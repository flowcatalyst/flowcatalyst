...
import { addSpecVersion, findSpecVersion, createSpecVersion, SchemaAdded } from '../../../domain/index.js';
...
export function createAddSchemaUseCase(
	...
	return {
		async execute(
			...
			const versionResult = validateRequired(command.version, 'version', 'VERSION_REQUIRED');
			if (Result.isFailure(versionResult)) return versionResult;

			if (!VERSION_PATTERN.test(command.version)) {
				return Result.failure(
					UseCaseError.validation('INVALID_VERSION', 'Version must be in MAJOR.MINOR format (e.g., "1.0")'),
				);
			}

			// Validate required fields
			const mimeResult = validateRequired(command.mimeType, 'mimeType', 'MIME_TYPE_REQUIRED');
			if (Result.isFailure(mimeResult)) return mimeResult;

			if (command.schemaContent === null || command.schemaContent === undefined) {
				return Result.failure(
					UseCaseError.validation('SCHEMA_REQUIRED', 'Schema content is required'),
				);
			}
			...
			if (eventType.status === 'ARCHIVED') {
				return Result.failure(
					UseCaseError.businessRule('EVENT_TYPE_ARCHIVED', 'Cannot add schema to an archived event type'),
				);
			}
			...
			if (existing) {
				return Result.failure(
					UseCaseError.businessRule('VERSION_EXISTS', 'Schema version already exists', {
