...
export function createFinaliseSchemaUseCase(
	...
	return {
		async execute(
			...
			let updated = updateSpecVersion(eventType, command.version, (sv) => withStatus(sv, 'CURRENT'));
			...
			for (const sv of eventType.specVersions) {
				if (
					...
					updated = updateSpecVersion(updated, sv.version, (s) => withStatus(s, 'DEPRECATED'));
