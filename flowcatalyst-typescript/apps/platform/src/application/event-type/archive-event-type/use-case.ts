...
import { archiveEventType, allVersionsDeprecated, EventTypeArchived } from '../../../domain/index.js';
	...
	return {
		async execute(
			...
			if (eventType.status === 'ARCHIVED') {
				return Result.failure(
					UseCaseError.businessRule('ALREADY_ARCHIVED', 'Event type is already archived'),
				);
			}
