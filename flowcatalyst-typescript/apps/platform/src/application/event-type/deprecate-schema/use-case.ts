...
import { findSpecVersion, updateSpecVersion, withStatus, SchemaDeprecated } from '../../../domain/index.js';
...
export function createDeprecateSchemaUseCase(
	...
	return {
		async execute(
			...
			if (!specVersion) {
				return Result.failure(
					UseCaseError.notFound('VERSION_NOT_FOUND', 'Schema version not found', {
			...
			if (specVersion.status === 'DEPRECATED') {
				return Result.failure(
					UseCaseError.businessRule('ALREADY_DEPRECATED', 'Schema is already deprecated'),
				);
			}

			const updated = updateSpecVersion(eventType, command.version, (sv) => withStatus(sv, 'DEPRECATED'));
