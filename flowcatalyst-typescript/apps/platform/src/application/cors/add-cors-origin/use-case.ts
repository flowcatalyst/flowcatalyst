...
export function createAddCorsOriginUseCase(
	...
	return {
		async execute(
			...
			const originPattern = /^https?:\/\/[a-zA-Z0-9]([a-zA-Z0-9\-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9\-]*[a-zA-Z0-9])?)*(:\d+)?$/;
			if (!originPattern.test(command.origin)) {
				return Result.failure(
					UseCaseError.validation('INVALID_ORIGIN', 'Origin must be a valid URL origin (e.g., https://example.com)'),
				);
			}
