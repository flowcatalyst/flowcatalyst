...
import type { IdentityProviderRepository, EmailDomainMappingRepository } from '../../../infrastructure/persistence/index.js';
	...
	return {
		async execute(
			...
			const updated = updateEmailDomainMapping(mapping, {
				...(command.identityProviderId !== undefined ? { identityProviderId: command.identityProviderId } : {}),
				...(command.scopeType !== undefined ? { scopeType: command.scopeType } : {}),
				...(command.primaryClientId !== undefined ? { primaryClientId: command.primaryClientId } : {}),
				...(command.additionalClientIds !== undefined ? { additionalClientIds: command.additionalClientIds } : {}),
				...(command.grantedClientIds !== undefined ? { grantedClientIds: command.grantedClientIds } : {}),
				...(command.requiredOidcTenantId !== undefined ? { requiredOidcTenantId: command.requiredOidcTenantId } : {}),
				...(command.allowedRoleIds !== undefined ? { allowedRoleIds: command.allowedRoleIds } : {}),
				...(command.syncRolesFromIdp !== undefined ? { syncRolesFromIdp: command.syncRolesFromIdp } : {}),
			});
