...
export function createCreateEmailDomainMappingUseCase(
	...
	return {
		async execute(
			...
			const idpResult = validateRequired(command.identityProviderId, 'identityProviderId', 'IDENTITY_PROVIDER_REQUIRED');
			...
			const domainExists = await emailDomainMappingRepository.existsByEmailDomain(command.emailDomain);
			if (domainExists) {
				return Result.failure(
