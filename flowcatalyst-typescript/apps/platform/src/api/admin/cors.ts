...
import type { AddCorsOriginCommand, DeleteCorsOriginCommand } from '../../application/index.js';
...
export interface CorsRoutesDeps {
  ...
  readonly deleteCorsOriginUseCase: UseCase<DeleteCorsOriginCommand, CorsOriginDeleted>;
}
...
export async function registerCorsRoutes(
  ...
  const { corsAllowedOriginRepository, addCorsOriginUseCase, deleteCorsOriginUseCase } = deps;
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (Result.isSuccess(result)) {
        const origin = await corsAllowedOriginRepository.findById(result.value.getData().originId);
        if (origin) {
