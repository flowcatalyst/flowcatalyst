...
import type { DispatchJobReadRecord, DispatchJobAttemptRecord } from '@flowcatalyst/persistence';
...
type DispatchJobAttemptResponse = Static<typeof DispatchJobAttemptResponseSchema>;
...
function toAttemptResponse(record: DispatchJobAttemptRecord): DispatchJobAttemptResponse {
	return {
		id: record.id,
		...
		async (request, reply) => {
			...
			if (accessibleClientIds !== null) {
				if (requestedClientIds) {
					scopedClientIds = requestedClientIds.filter((id) => accessibleClientIds.includes(id));
				} else {
