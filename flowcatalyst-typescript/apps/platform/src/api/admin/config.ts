...
import type { PlatformConfigService, ConfigScope, ConfigValueType } from '../../domain/index.js';

// ─── Request Schemas ────────────────────────────────────────────────────────

const SetConfigSchema = Type.Object({
	value: Type.String(),
	valueType: Type.Optional(Type.Union([Type.Literal('PLAIN'), Type.Literal('SECRET')])),
	description: Type.Optional(Type.String()),
});

const AppCodeParam = Type.Object({ appCode: Type.String() });
const SectionParam = Type.Object({ appCode: Type.String(), section: Type.String() });
...
const ScopeQuery = Type.Object({
	scope: Type.Optional(Type.Union([Type.Literal('GLOBAL'), Type.Literal('CLIENT')])),
...
export async function registerConfigRoutes(
	...
	function getRoles(request: { audit?: { principal?: { roles?: ReadonlySet<string> } | null } }): readonly string[] {
	...
	fastify.get(
		...
		async (request, reply) => {
			...
			const hasAccess = await platformConfigService.canAccess(appCode, roles, false);
			if (!hasAccess) {
				return forbidden(reply, 'Access denied to configuration');
			}

			const configs = await platformConfigService.getConfigs(appCode, scope, clientId);
	...
	fastify.get(
		...
		async (request, reply) => {
			const { appCode, section } = request.params as Static<typeof SectionParam>;
			const query = request.query as Static<typeof ScopeQuery>;
			const { scope, clientId } = parseScope(query);
			const roles = getRoles(request);
			...
			const values = await platformConfigService.getSection(appCode, section, scope, clientId);
	...
	fastify.get(
		...
		async (request, reply) => {
			const { appCode, section, property } = request.params as Static<typeof PropertyParam>;
			const query = request.query as Static<typeof ScopeQuery>;
			const { scope, clientId } = parseScope(query);
			const roles = getRoles(request);
			...
			const value = await platformConfigService.getValue(appCode, section, property, scope, clientId);
			if (value === undefined) {
				return notFound(reply, `Config not found: ${appCode}.${section}.${property}`);
			}
	...
	fastify.put(
		...
		async (request, reply) => {
			...
			const hasAccess = await platformConfigService.canAccess(appCode, roles, true);
			if (!hasAccess) {
			...
			if (scope === 'GLOBAL' && clientId) {
				return badRequest(reply, 'clientId must not be provided for GLOBAL scope');
			}

			// Check if this is a create or update
			const existing = await platformConfigService.getValue(appCode, section, property, scope, clientId);
	...
	fastify.delete(
		...
		async (request, reply) => {
			...
			const deleted = await platformConfigService.deleteValue(appCode, section, property, scope, clientId);
			if (!deleted) {
				return notFound(reply, `Config not found: ${appCode}.${section}.${property}`);
			}
