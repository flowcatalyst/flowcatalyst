...
import { createPlatformConfigAccess, type PlatformConfigAccess } from '../../domain/index.js';

// ─── Request Schemas ────────────────────────────────────────────────────────

const AppCodeParam = Type.Object({ appCode: Type.String() });
const RoleParam = Type.Object({ appCode: Type.String(), roleCode: Type.String() });
...
export async function registerConfigAccessRoutes(
	...
	fastify.get(
		...
		async (request, reply) => {
			...
			const grants = await platformConfigAccessRepository.findByApplication(appCode);
	...
	fastify.post(
		...
		async (request, reply) => {
			...
			const existing = await platformConfigAccessRepository.findByApplicationAndRole(
				appCode,
				body.roleCode,
			);
			if (existing) {
				return jsonError(reply, 409, 'GRANT_EXISTS', `Access grant already exists for role: ${body.roleCode}`);
			}
	...
	fastify.put(
		...
		async (request, reply) => {
			...
			const existing = await platformConfigAccessRepository.findByApplicationAndRole(appCode, roleCode);
			if (!existing) {
	...
	fastify.delete(
		...
		async (request, reply) => {
			...
			const deleted = await platformConfigAccessRepository.deleteByApplicationAndRole(appCode, roleCode);
			if (!deleted) {
				return notFound(reply, `Access grant not found for role: ${roleCode}`);
			}
...
function toAccessGrantResponse(grant: PlatformConfigAccess): AccessGrantResponse {
	return {
		id: grant.id,
