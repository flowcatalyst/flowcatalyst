...
import type {
	PrincipalRepository,
} from '../../infrastructure/persistence/repositories/principal-repository.js';
import type {
	ClientAccessGrantRepository,
} from '../../infrastructure/persistence/repositories/client-access-grant-repository.js';
import type {
	AnchorDomainRepository,
} from '../../infrastructure/persistence/repositories/anchor-domain-repository.js';
import type {
	ClientAuthConfigRepository,
} from '../../infrastructure/persistence/repositories/client-auth-config-repository.js';
import type {
	ApplicationClientConfigRepository,
} from '../../infrastructure/persistence/repositories/application-client-config-repository.js';
 ...
 */
export async function registerPrincipalsRoutes(fastify: FastifyInstance, deps: PrincipalsRoutesDeps): Promise<void> {
	const {
		principalRepository,
		clientAccessGrantRepository,
		anchorDomainRepository,
	...
	fastify.put(
		...
		async (request, reply) => {
			...
			if (Result.isSuccess(result)) {
				...
				if (principal) {
					const grants = await clientAccessGrantRepository.findByPrincipal(id);
					return jsonSuccess(reply, toPrincipalDto(principal, grants.map((g) => g.clientId)));
				}
			}
	...
	fastify.post(
		...
		async (request, reply) => {
			...
			if (!principal.userIdentity || principal.userIdentity.idpType !== 'INTERNAL') {
				return badRequest(reply, 'Password reset is only supported for internal authentication users');
			}
