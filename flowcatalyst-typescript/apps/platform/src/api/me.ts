...
import { jsonSuccess, notFound, forbidden, ErrorResponseSchema } from '@flowcatalyst/http';
import { requireAuthentication } from '../authorization/index.js';
import { canAccessClient, getClientQueryScopeForPrincipal } from '../authorization/index.js';
import type { ClientRepository } from '../infrastructure/persistence/index.js';
import type { ApplicationRepository, ApplicationClientConfigRepository } from '../infrastructure/persistence/index.js';
...
export async function registerMeRoutes(fastify: FastifyInstance, deps: MeRoutesDeps): Promise<void> {
	const { clientRepository, applicationRepository, applicationClientConfigRepository } = deps;
	...
	fastify.get(
		...
		async (request, reply) => {
			...
			let clients: { id: string; name: string; identifier: string; status: string; createdAt: Date; updatedAt: Date }[];
			...
			switch (scope.type) {
				...
				case 'restricted':
					if (scope.filter.clientIds.length === 0) {
						...
						clients = results.filter((c): c is NonNullable<typeof c> => c != null);
					}
					break;
				case 'denied':
					clients = [];
	...
	fastify.get(
		...
		async (request, reply) => {
			...
			const configs = await applicationClientConfigRepository.findByClient(clientId);
			const enabledConfigsByAppId = new Map(
				configs
					.filter((c) => c.enabled)
					.map((c) => [c.applicationId, c]),
			);
