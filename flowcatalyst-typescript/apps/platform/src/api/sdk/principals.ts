 ...
 */
export async function registerSdkPrincipalsRoutes(fastify: FastifyInstance, deps: SdkPrincipalsDeps): Promise<void> {
	const {
		principalRepository,
		clientAccessGrantRepository,
		createUserUseCase,
	...
	fastify.get(
		...
		async (request, reply) => {
			...
			if (query.email) {
				...
				principals = await principalRepository.findByType(query.type.toUpperCase() as PrincipalType);
			} else {
			...
			const items = await Promise.all(
				principals.map(async (p) => {
					const grants = await clientAccessGrantRepository.findByPrincipal(p.id);
					return toSdkPrincipal(p, grants.map((g) => g.clientId));
				}),
			);
	...
	fastify.get(
		...
		async (request, reply) => {
			...
			return jsonSuccess(reply, toSdkPrincipal(principal, grants.map((g) => g.clientId)));
		},
	);
	...
	fastify.post(
		...
		async (request, reply) => {
			...
			if (Result.isSuccess(result)) {
				const grant = await clientAccessGrantRepository.findByPrincipalAndClient(id, clientId);
				if (grant) {
...
function toSdkPrincipal(principal: Principal, grantedClientIds: string[]): SdkPrincipalResponse {
	return {
		id: principal.id,
		type: principal.type,
