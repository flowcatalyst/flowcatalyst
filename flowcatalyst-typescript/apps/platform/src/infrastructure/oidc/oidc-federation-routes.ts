...
import { isValidIssuer, isEmailDomainAllowed } from '../../domain/identity-provider/identity-provider.js';
...
import type { OidcLoginStateRepository, OidcLoginState } from '../persistence/repositories/oidc-login-state-repository.js';
...
export async function registerOidcFederationRoutes(
	...
	fastify.get<{
		...
		if (!mapping) {
			return reply.status(404).send({ error: `No authentication configuration found for domain: ${domain}` });
		}
		...
		if (idp.type !== 'OIDC') {
			return reply.status(400).send({ error: `Domain ${domain} uses internal authentication, not OIDC` });
		}

		if (!idp.oidcIssuerUrl || !idp.oidcClientId) {
			return reply.status(500).send({ error: `OIDC configuration incomplete for domain: ${domain}` });
		}

		// Validate email domain is allowed by IDP
		if (!isEmailDomainAllowed(idp, domain)) {
			return reply.status(403).send({ error: `Email domain ${domain} is not allowed by this identity provider` });
		}
	...
	fastify.get<{
		...
		const mapping = await deps.emailDomainMappingRepository.findById(loginState.emailDomainMappingId);
		if (!mapping) {
		...
		try {
			...
			if (idp.oidcMultiTenant) {
				if (!mapping.requiredOidcTenantId) {
					...
					return errorRedirect(reply, deps, 'Configuration error: tenant ID required for this identity provider');
				}
				if (!tenantId) {
					fastify.log.warn({ emailDomain }, 'Tenant ID required but not in token');
					return errorRedirect(reply, deps, 'Authentication failed: tenant information not provided');
				}
			...
			if (mapping.syncRolesFromIdp) {
				...
				if (idpRoleNames.length > 0) {
					...
					const roleResult = await syncIdpRoles(principal, idpRoleNames, allowedNames, ctx, syncDeps);
			...
			fastify.log.error({ err, emailDomain: loginState.emailDomain }, 'OIDC callback processing failed');
...
async function determineAccessibleClients(
	principal: { scope: string | null; clientId: string | null; roles: readonly { roleName: string }[] },
	roles: string[],
 ...
 */
async function formatClientEntries(clientIds: string[], deps: OidcFederationDeps): Promise<string[]> {
...
function determineRedirectUrl(loginState: OidcLoginState, baseUrl: string): string {
	...
	if (loginState.oauthClientId) {
		...
		if (loginState.oauthCodeChallengeMethod) params.set('code_challenge_method', loginState.oauthCodeChallengeMethod);
