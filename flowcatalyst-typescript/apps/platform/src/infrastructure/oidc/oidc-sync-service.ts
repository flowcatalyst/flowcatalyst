...
export async function createOrUpdateOidcUser(
	...
	return deps.unitOfWork.commit(newPrincipal, event, { _type: 'OidcUserCreate', email, externalIdpId });
}
...
export async function syncIdpRoles(
	...
	if (!idpRoleNames || idpRoleNames.length === 0) {
		...
		for (const idpRoleName of idpRoleNames) {
			...
			if (mapping) {
				...
				deps.log.debug(
					{ principalId: principal.id, idpRole: idpRoleName, internalRole: mapping.internalRoleName },
					'Accepted IDP role',
				);
			} else {
	...
	return deps.unitOfWork.commit(updated, event, { _type: 'OidcIdpRoleSync', principalId: principal.id });
}
