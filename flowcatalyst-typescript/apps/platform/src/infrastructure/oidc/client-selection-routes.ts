...
export async function registerClientSelectionRoutes(
	...
	fastify.get('/auth/client/accessible', async (request, reply) => {
		const principal = await authenticatePrincipal(request, validateSessionToken, principalRepository, cookieConfig);
		if (!principal) {
		...
		const accessibleClientIds = await getAccessibleClientIds(principal, emailDomainMappingRepository);
	...
	fastify.post<{ Body: SwitchClientRequest }>('/auth/client/switch', async (request, reply) => {
		...
		const clientsForToken = accessibleClientIds === null
			? ['*']
	...
	fastify.log.info('Client selection routes registered (/auth/client/accessible, /auth/client/switch, /auth/client/current)');
}
...
async function authenticatePrincipal(
	request: { cookies: Record<string, string | undefined>; headers: { authorization?: string | undefined } },
	validateSessionToken: (token: string) => Promise<string | null>,
	principalRepository: PrincipalRepository,
	cookieConfig: SessionCookieConfig,
) {
...
async function getAccessibleClientIds(
	...
	if (principal.userIdentity?.emailDomain) {
		const mapping = await emailDomainMappingRepository.findByEmailDomain(principal.userIdentity.emailDomain);
		if (mapping) {
