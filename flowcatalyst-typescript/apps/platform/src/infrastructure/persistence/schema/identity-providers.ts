...
export const identityProviders = pgTable(
	...
	(table) => [
		uniqueIndex('idx_identity_providers_code').on(table.code),
	],
);
...
export const identityProviderAllowedDomains = pgTable(
	...
	(table) => [
		index('idx_idp_allowed_domains_idp').on(table.identityProviderId),
	],
);

export type IdentityProviderAllowedDomainRecord = typeof identityProviderAllowedDomains.$inferSelect;
export type NewIdentityProviderAllowedDomainRecord = typeof identityProviderAllowedDomains.$inferInsert;
