...
export const platformConfigs = pgTable(
	...
	(table) => [
		unique('uq_platform_config_key').on(
		...
		index('idx_platform_configs_lookup').on(
			table.applicationCode,
			table.section,
			table.scope,
			table.clientId,
		),
		index('idx_platform_configs_app_section').on(table.applicationCode, table.section),
	],
);
...
export const platformConfigAccess = pgTable(
	...
	(table) => [
		...
		index('idx_config_access_app').on(table.applicationCode),
		index('idx_config_access_role').on(table.roleCode),
	],
);
