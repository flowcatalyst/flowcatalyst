...
export interface SubscriptionRepository extends Repository<Subscription> {
	...
	existsByCodeAndClient(code: string, clientId: string | null, tx?: TransactionContext): Promise<boolean>;
...
export function createSubscriptionRepository(defaultDb: AnyDb): SubscriptionRepository {
	 ...
	 */
	async function loadEventTypes(subscriptionId: string, txCtx?: TransactionContext): Promise<EventTypeBinding[]> {
	 ...
	 */
	async function loadCustomConfig(subscriptionId: string, txCtx?: TransactionContext): Promise<ConfigEntry[]> {
		const records = await db(txCtx)
			.select()
	 ...
	 */
	async function hydrate(record: SubscriptionRecord, txCtx?: TransactionContext): Promise<Subscription> {
	...
	return {
		...
		async findByDispatchPoolId(dispatchPoolId: string, tx?: TransactionContext): Promise<Subscription[]> {
			const records = await db(tx)
				.select()
				.from(subscriptions)
		...
		async existsByDispatchPoolId(dispatchPoolId: string, tx?: TransactionContext): Promise<boolean> {
			const [result] = await db(tx)
				.select({ count: sql<number>`count(*)` })
				.from(subscriptions)
				.where(eq(subscriptions.dispatchPoolId, dispatchPoolId));
		...
		async findWithFilters(filters: SubscriptionFilters, tx?: TransactionContext): Promise<Subscription[]> {
			const conditions = [];
		...
		async deleteById(id: string, tx?: TransactionContext): Promise<boolean> {
			...
			await db(tx).delete(subscriptionEventTypes).where(eq(subscriptionEventTypes.subscriptionId, id));
			await db(tx).delete(subscriptionCustomConfigs).where(eq(subscriptionCustomConfigs.subscriptionId, id));
