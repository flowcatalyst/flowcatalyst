...
export interface IdpRoleMappingRepository {
	findById(id: string, tx?: TransactionContext): Promise<IdpRoleMapping | undefined>;
	findByIdpRoleName(idpRoleName: string, tx?: TransactionContext): Promise<IdpRoleMapping | undefined>;
	findAll(tx?: TransactionContext): Promise<IdpRoleMapping[]>;
	insert(entity: Omit<IdpRoleMapping, 'createdAt' | 'updatedAt'>, tx?: TransactionContext): Promise<void>;
	deleteById(id: string, tx?: TransactionContext): Promise<boolean>;
}

export function createIdpRoleMappingRepository(defaultDb: AnyDb): IdpRoleMappingRepository {
	const db = (tx?: TransactionContext): AnyDb => (tx?.db as AnyDb) ?? defaultDb;

	function hydrate(record: typeof idpRoleMappings.$inferSelect): IdpRoleMapping {
		return {
			id: record.id,
	...
	return {
		async findById(id: string, tx?: TransactionContext): Promise<IdpRoleMapping | undefined> {
		...
		async findByIdpRoleName(idpRoleName: string, tx?: TransactionContext): Promise<IdpRoleMapping | undefined> {
			const [record] = await db(tx)
				.select()
				.from(idpRoleMappings)
		...
		async insert(entity: Omit<IdpRoleMapping, 'createdAt' | 'updatedAt'>, tx?: TransactionContext): Promise<void> {
			const now = new Date();
