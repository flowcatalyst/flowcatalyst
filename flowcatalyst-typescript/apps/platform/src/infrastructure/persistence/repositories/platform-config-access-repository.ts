...
import { type PlatformConfigAccess, type NewPlatformConfigAccess } from '../../../domain/index.js';

/**
 * Platform config access repository interface.
 */
export interface PlatformConfigAccessRepository {
	findByApplication(applicationCode: string, tx?: TransactionContext): Promise<PlatformConfigAccess[]>;
	...
	insert(entity: NewPlatformConfigAccess, tx?: TransactionContext): Promise<PlatformConfigAccess>;
	update(entity: PlatformConfigAccess, tx?: TransactionContext): Promise<PlatformConfigAccess>;
 ...
 */
export function createPlatformConfigAccessRepository(defaultDb: AnyDb): PlatformConfigAccessRepository {
	...
	return {
		...
		async insert(
			...
			const inserted = await this.findByApplicationAndRole(entity.applicationCode, entity.roleCode, tx);
			return inserted!;
		},
		...
		async update(
			...
			const updated = await this.findByApplicationAndRole(entity.applicationCode, entity.roleCode, tx);
			return updated!;
		},
		...
		async deleteByApplicationAndRole(
			...
			const existing = await this.findByApplicationAndRole(applicationCode, roleCode, tx);
...
function recordToEntity(record: PlatformConfigAccessRecord): PlatformConfigAccess {
	return {
		id: record.id,
