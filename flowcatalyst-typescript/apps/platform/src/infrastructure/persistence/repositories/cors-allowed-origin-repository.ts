...
import { type CorsAllowedOrigin, type NewCorsAllowedOrigin } from '../../../domain/index.js';

/**
 * CORS allowed origin repository interface.
 */
export interface CorsAllowedOriginRepository {
	findById(id: string, tx?: TransactionContext): Promise<CorsAllowedOrigin | undefined>;
	findByOrigin(origin: string, tx?: TransactionContext): Promise<CorsAllowedOrigin | undefined>;
	...
	insert(entity: NewCorsAllowedOrigin, tx?: TransactionContext): Promise<CorsAllowedOrigin>;
	update(entity: CorsAllowedOrigin, tx?: TransactionContext): Promise<CorsAllowedOrigin>;
	persist(entity: NewCorsAllowedOrigin, tx?: TransactionContext): Promise<CorsAllowedOrigin>;
 ...
 */
export function createCorsAllowedOriginRepository(defaultDb: AnyDb): CorsAllowedOriginRepository {
	const db = (tx?: TransactionContext): AnyDb => (tx?.db as AnyDb) ?? defaultDb;

	return {
		async findById(id: string, tx?: TransactionContext): Promise<CorsAllowedOrigin | undefined> {
		...
		async findByOrigin(origin: string, tx?: TransactionContext): Promise<CorsAllowedOrigin | undefined> {
			const [record] = await db(tx)
				.select()
				.from(corsAllowedOrigins)
		...
		async existsByOrigin(origin: string, tx?: TransactionContext): Promise<boolean> {
			const [result] = await db(tx)
				.select({ count: sql<number>`count(*)` })
				.from(corsAllowedOrigins)
				.where(eq(corsAllowedOrigins.origin, origin.toLowerCase()));
		...
		async insert(entity: NewCorsAllowedOrigin, tx?: TransactionContext): Promise<CorsAllowedOrigin> {
			const now = new Date();
		...
		async update(entity: CorsAllowedOrigin, tx?: TransactionContext): Promise<CorsAllowedOrigin> {
			const now = new Date();
			await db(tx)
		...
		async persist(entity: NewCorsAllowedOrigin, tx?: TransactionContext): Promise<CorsAllowedOrigin> {
		...
		async deleteById(id: string, tx?: TransactionContext): Promise<boolean> {
			...
			await db(tx).delete(corsAllowedOrigins).where(eq(corsAllowedOrigins.id, id));
			return true;
		},

		async delete(entity: CorsAllowedOrigin, tx?: TransactionContext): Promise<boolean> {
