...
import { dispatchPools, type DispatchPoolRecord, type NewDispatchPoolRecord } from '../schema/index.js';
import type { DispatchPool, NewDispatchPool, DispatchPoolStatus } from '../../../domain/index.js';
...
export interface DispatchPoolRepository extends Repository<DispatchPool> {
	...
	existsByCodeAndClientId(code: string, clientId: string | null, tx?: TransactionContext): Promise<boolean>;
	findAnchorLevel(tx?: TransactionContext): Promise<DispatchPool[]>;
	findByClientId(clientId: string, tx?: TransactionContext): Promise<DispatchPool[]>;
	findActive(tx?: TransactionContext): Promise<DispatchPool[]>;
	findWithFilters(filters: DispatchPoolFilters, tx?: TransactionContext): Promise<DispatchPool[]>;
}

/**
 * Create a Dispatch Pool repository.
 */
export function createDispatchPoolRepository(defaultDb: AnyDb): DispatchPoolRepository {
	const db = (tx?: TransactionContext): AnyDb => (tx?.db as AnyDb) ?? defaultDb;

	return {
		async findById(id: string, tx?: TransactionContext): Promise<DispatchPool | undefined> {
		...
		async findByCodeAndClientId(
			...
			const condition =
				clientId === null
					...
					: and(eq(dispatchPools.code, code), eq(dispatchPools.clientId, clientId));

			const [record] = await db(tx).select().from(dispatchPools).where(condition).limit(1);
		...
		async findAll(tx?: TransactionContext): Promise<DispatchPool[]> {
			const records = await db(tx).select().from(dispatchPools).orderBy(dispatchPools.code);
		...
		async findByClientId(clientId: string, tx?: TransactionContext): Promise<DispatchPool[]> {
			const records = await db(tx)
				.select()
				.from(dispatchPools)
		...
		async findWithFilters(filters: DispatchPoolFilters, tx?: TransactionContext): Promise<DispatchPool[]> {
			const conditions = [];
		...
		async insert(entity: NewDispatchPool, tx?: TransactionContext): Promise<DispatchPool> {
			const now = new Date();
		...
		async update(entity: DispatchPool, tx?: TransactionContext): Promise<DispatchPool> {
			const now = new Date();
		...
		async persist(entity: NewDispatchPool, tx?: TransactionContext): Promise<DispatchPool> {
		...
		async delete(entity: DispatchPool, tx?: TransactionContext): Promise<boolean> {
