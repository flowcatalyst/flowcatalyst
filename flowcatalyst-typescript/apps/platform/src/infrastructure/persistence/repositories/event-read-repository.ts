...
export interface EventReadRepository {
	findById(id: string, tx?: TransactionContext): Promise<EventReadRecord | undefined>;
	findPaged(filters: EventReadFilters, pagination: EventReadPagination, tx?: TransactionContext): Promise<PagedEventReadResult>;
	getFilterOptions(request: EventFilterOptionsRequest, tx?: TransactionContext): Promise<EventFilterOptions>;
 ...
 */
export function createEventReadRepository(defaultDb: AnyDb): EventReadRepository {
	const db = (tx?: TransactionContext): AnyDb => (tx?.db as AnyDb) ?? defaultDb;

	function buildConditions(filters: EventReadFilters | EventFilterOptionsRequest) {
		const conditions = [];
	...
	return {
		async findById(id: string, tx?: TransactionContext): Promise<EventReadRecord | undefined> {
		...
		async findPaged(
			...
			const baseCount = db(tx).select({ count: sql<number>`count(*)` }).from(eventsRead);

			const [records, countResult] = await Promise.all([
				whereClause
					? baseSelect.where(whereClause).orderBy(desc(eventsRead.time)).limit(size).offset(offset)
					: baseSelect.orderBy(desc(eventsRead.time)).limit(size).offset(offset),
				whereClause
		...
		async getFilterOptions(
			...
			const [appResults, subResults, aggResults, typeResults] = await Promise.all([
				whereClause
					? queryDb.selectDistinct({ value: eventsRead.application }).from(eventsRead).where(whereClause)
					: queryDb.selectDistinct({ value: eventsRead.application }).from(eventsRead),
				whereClause
					? queryDb.selectDistinct({ value: eventsRead.subdomain }).from(eventsRead).where(whereClause)
					: queryDb.selectDistinct({ value: eventsRead.subdomain }).from(eventsRead),
				whereClause
					? queryDb.selectDistinct({ value: eventsRead.aggregate }).from(eventsRead).where(whereClause)
					: queryDb.selectDistinct({ value: eventsRead.aggregate }).from(eventsRead),
				whereClause
					? queryDb.selectDistinct({ value: eventsRead.type }).from(eventsRead).where(whereClause)
					: queryDb.selectDistinct({ value: eventsRead.type }).from(eventsRead),
			]);
