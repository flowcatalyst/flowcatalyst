-- backfill-iam-read-perms.sql — OPTIONAL drop-in helper (Go↔legacy-platform IAM parity).
--
-- WHAT THIS IS FOR
...
-- only ever sees principals in clients they can access. The legacy platform gated
...
-- on every boot, so they self-heal when Go is dropped in over a legacy-platform DB.
--
-- The ONLY residual gap: a NON-anchor, client-scoped principal who on legacy-platform could
...
-- are platform-administration reads (anchor/admin only on both Go and legacy platform), so
...
-- safe to backfill: the filter guarantees no cross-tenant leak (matches the wire contract).
--
...
-- view-perm holder, whereas the legacy implementation shows those only to anchors. If that matters,
