...
func main() {
	v, err := webhook.ValidatorFromEnv() // reads FLOWCATALYST_SIGNING_SECRET
	if err != nil {
	...
	mux.HandleFunc("POST /webhook", func(w http.ResponseWriter, r *http.Request) {
		...
		sig := r.Header.Get(webhook.ValidatorSignatureHeader)
		ts := r.Header.Get(webhook.ValidatorTimestampHeader)

		if err := v.Validate(sig, ts, body); err != nil {
			// errors.Is is the convention — every webhook error is a
			// declared sentinel.
			switch {
			case errors.Is(err, webhook.ErrValidatorMissingSignature),
				errors.Is(err, webhook.ErrValidatorMissingTimestamp),
				errors.Is(err, webhook.ErrValidatorInvalidTimestamp):
				http.Error(w, err.Error(), http.StatusBadRequest)
			case errors.Is(err, webhook.ErrTimestampExpired),
				errors.Is(err, webhook.ErrTimestampInFuture):
				http.Error(w, err.Error(), http.StatusUnauthorized)
			case errors.Is(err, webhook.ErrValidatorInvalidSignature):
