...
func TestValidateAcceptsFreshSignedRequest(t *testing.T) {
	v := webhook.NewValidator("test-secret")
...
func TestValidateRejectsBadSignature(t *testing.T) {
	...
	body := []byte(`{"type":"order.created"}`)
	ts := strconv.FormatInt(time.Now().UTC().Unix(), 10)
	_ = v.ComputeSignature(ts, body)
	err := v.Validate("deadbeef", ts, body)
	assert.True(t, errors.Is(err, webhook.ErrValidatorInvalidSignature))
}
...
func TestValidateRejectsMissingHeaders(t *testing.T) {
	v := webhook.NewValidator("test-secret")
	assert.True(t, errors.Is(v.Validate("", "1234", nil), webhook.ErrValidatorMissingSignature))
	assert.True(t, errors.Is(v.Validate("sig", "", nil), webhook.ErrValidatorMissingTimestamp))
	assert.True(t, errors.Is(v.Validate("sig", "notanumber", nil), webhook.ErrValidatorInvalidTimestamp))
}
