...
type CreateUserRequest struct {
	...
	// the calling application's own page, which then starts its own sign-in;
	// it is not an OAuth redirect URI. It rides on the invite token, so it
	// applies to both delivery modes: the platform-sent invite email and
	// returnInviteLink's link. Any absolute http(s) URL is accepted; a
	// malformed value rejects the whole request before the user is created
	// (INVITE_REDIRECT_URI_INVALID). Ignored when no invite is minted (a user
	// created with a password, a federated user, or sendInvitation:false
	// without returnInviteLink). Without it the invitee lands in the platform
	// console.
	InviteRedirectURI *string `json:"inviteRedirectUri,omitempty"`
}
