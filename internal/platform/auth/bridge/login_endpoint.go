...
func (e *LoginEndpoint) handleCallback(w http.ResponseWriter, r *http.Request) {
	...
	// guarantees a portal IdP can never grant platform roles — the role-sync
	// config now lives on the IdP itself, which a provider-direct login DOES
	// carry, so the guard is what keeps portal logins out of it.
	if !providerDirect {
