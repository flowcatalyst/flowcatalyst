...
// RegisterRoutes mounts the IDP endpoints. Anchor-only.
func RegisterRoutes(r chi.Router, s *State) {
	r.Route("/api/identity-providers", func(r chi.Router) {
		r.Get("/", s.list)
		r.Post("/", s.create)
		r.Get("/{id}", s.getByID)
		r.Put("/{id}", s.update)
...
func (s *State) getByID(w http.ResponseWriter, r *http.Request) {
	...
	ip, err := s.Repo.FindByID(r.Context(), id)
	if err != nil {
		httperror.Write(w, usecase.Internal("REPO", "find_by_id failed", err))
		return
	}
	if ip == nil {
		httperror.Write(w, httperror.NotFound("IdentityProvider", id))
		return
	}
	w.Header().Set("Content-Type", "application/json")
...
func (s *State) create(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(apicommon.CreatedResponse{ID: event.IdentityProviderID})
}
