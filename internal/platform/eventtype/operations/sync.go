...
// All event writes are atomic with the row writes via [usecaseop.Sync].
//
// Authorization is intentionally [usecaseop.Public]: this op is reached by two
// entry points with different gating — the app-scoped SDK sync (CanSyncEventTypes
// + per-application access) and the anchor-only BFF platform-catalogue sync
// (which uses a "platform" pseudo-code, not a real application). Each entry
// point keeps its own gate; baking either into the op would break the other.
func SyncEventTypes(repo *eventtype.Repository) usecaseop.Operation[SyncEventTypesCommand, EventTypesSynced] {
	return usecaseop.Operation[SyncEventTypesCommand, EventTypesSynced]{
		Name: "SyncEventTypes",
		Validate: func(_ context.Context, cmd SyncEventTypesCommand) error {
			if strings.TrimSpace(cmd.ApplicationCode) == "" {
				return usecase.Validation("APPLICATION_CODE_REQUIRED", "Application code is required")
			}
			return nil
		},
		Authorize: usecaseop.Public[SyncEventTypesCommand],
		Execute: func(ctx context.Context, cmd SyncEventTypesCommand, ec usecase.ExecutionContext) (usecaseop.Plan[EventTypesSynced], error) {
			...
			var (
				saves       []usecasepgx.SyncSaveItem[eventtype.EventType]
				deletes     []usecasepgx.SyncDeleteItem[eventtype.EventType]
			...
			for _, in := range cmd.EventTypes {
				...
				if err != nil {
					...
					return nil, usecase.Validation("INVALID_CODE",
			...
			if cmd.RemoveUnlisted {
				for code, cur := range existingByCode {
					...
					deletes = append(deletes, usecasepgx.SyncDeleteItem[eventtype.EventType]{
						Aggregate: cur,
