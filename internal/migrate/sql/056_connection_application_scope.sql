...
-- / .source, migration 004), so a later code-first sync can tell a
-- code/API-authored connection apart from a UI-authored one and refuse to
-- touch the latter — exactly the rule SyncSubscriptions already enforces for
-- subscriptions (see subscription/operations/sync.go).
--
-- application_code matches msg_subscriptions.application_code's type
-- exactly (VARCHAR(100), nullable, no FK — app_applications.code is not
...
-- connection.Source — internal/platform/connection/entity.go. Same guarded
-- CHECK style as the rest of the X-06 enum constraints (migration 051):
-- matches the Go const block exactly, and idempotent via the pg_constraint
-- existence check (no ADD CONSTRAINT IF NOT EXISTS in PostgreSQL).
...
-- idx_msg_subscriptions_code_client indexes are plain UNIQUE (code,
-- client_id): Postgres treats NULLs as distinct in a unique index, so two
-- rows with the same code and both a NULL client_id were never actually
-- rejected by them — a latent gap this migration closes at the same time it
-- adds the application dimension.
--
...
-- +goose Down
DROP INDEX IF EXISTS uq_msg_subscriptions_app_client_code;
DROP INDEX IF EXISTS uq_msg_connections_app_client_code;
...
ALTER TABLE msg_connections DROP CONSTRAINT IF EXISTS chk_msg_connections_source;
ALTER TABLE msg_connections DROP COLUMN IF EXISTS source;
ALTER TABLE msg_connections DROP COLUMN IF EXISTS application_code;
