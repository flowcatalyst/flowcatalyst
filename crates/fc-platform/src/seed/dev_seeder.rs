...
use crate::{
    AnchorDomain, Application, AuthProvider, AuthRole, Client, ClientAccessGrant, ClientAuthConfig,
    ClientStatus, EventType, Principal, RoleSource, UserScope,
};
use crate::{
    AnchorDomainRepository, ApplicationRepository, ClientAccessGrantRepository,
    ClientAuthConfigRepository, ClientRepository, EventTypeRepository, PrincipalRepository,
    RoleRepository,
};
...
impl DevDataSeeder {
    pub fn new(pg_pool: PgPool) -> Self {
        ...
        let password_service =
            PasswordService::new(Argon2Config::testing(), PasswordPolicy::lenient());
        Self {
            pg_pool,
            password_service,
        }
    ...
    pub async fn seed(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.seed_email_domain_mappings(&internal_idp_id, &clients)
            .await?;
        ...
        info!("Default logins:");
        info!(
            "  Platform Admin: admin@flowcatalyst.local / {}",
            DEV_PASSWORD
        );
    ...
    async fn seed_internal_identity_provider(&self) -> Result<String, Box<dyn std::error::Error>> {
        ...
        let idp = IdentityProvider::new(
            ...
            crate::identity_provider::entity::IdentityProviderType::Internal,
        );
    ...
    async fn create_edm_if_not_exists(
        ...
        info!(
            "Created email domain mapping: {} → internal ({})",
            domain,
            scope_type.as_str()
        );
    ...
    async fn seed_clients(&self) -> Result<SeedClients, Box<dyn std::error::Error>> {
        ...
        let acme = self
            .create_client_if_not_exists(&repo, "Acme Corporation", "acme", ClientStatus::Active)
            .await?;
        let globex = self
            .create_client_if_not_exists(&repo, "Globex Industries", "globex", ClientStatus::Active)
            .await?;
        let _initech = self
    ...
    async fn seed_auth_configs(
        &self,
        _clients: &SeedClients,
    ) -> Result<(), Box<dyn std::error::Error>> {
        let repo = ClientAuthConfigRepository::new(&self.pg_pool);

        self.create_auth_config_if_not_exists(&repo, "flowcatalyst.local")
            .await?;
        self.create_auth_config_if_not_exists(&repo, "acme.com")
            .await?;
        self.create_auth_config_if_not_exists(&repo, "globex.com")
            .await?;
        self.create_auth_config_if_not_exists(&repo, "initech.com")
            .await?;
        self.create_auth_config_if_not_exists(&repo, "partner.io")
            .await?;
    ...
    async fn seed_application_roles(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let roles: &[(&str, &str, &str, &str)] = &[
            (
                ...
                "client-admin",
                "Acme Client Admin",
                "Client-level administration for Acme",
            ),
            (
                "dispatch",
                "admin",
            ...
            (
                ...
                "user",
                "Dispatch User",
                "Create and manage dispatch jobs",
            ),
            (
                "dispatch",
                "viewer",
                "Dispatch Viewer",
                "Read-only access to dispatch data",
            ),
        ];
    ...
    async fn seed_users(&self, clients: &SeedClients) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let (Some(partner), Some(ref acme)) = (&partner, &clients.acme) {
            self.create_grant_if_not_exists(&grant_repo, &partner.id, &acme.id)
                .await?;
        }
        if let (Some(partner), Some(ref globex)) = (&partner, &clients.globex) {
            self.create_grant_if_not_exists(&grant_repo, &partner.id, &globex.id)
                .await?;
        }
    ...
    async fn create_grant_if_not_exists(
        ...
        if repo
            .find_by_principal_and_client(principal_id, client_id)
            .await?
            .is_some()
        {
            return Ok(());
    ...
    async fn seed_event_types(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.create_event_type(
            &repo,
            "tms:execution:shipment:dispatched",
            "Shipment Dispatched",
        ...
        self.create_event_type(
            &repo,
            "tms:execution:shipment:in-transit",
            "Shipment In Transit",
        ...
        self.create_event_type(
            &repo,
            "tms:execution:shipment:delivered",
            "Shipment Delivered",
        ...
        self.create_event_type(
            &repo,
            "tms:execution:shipment:exception",
            "Shipment Exception",
        ...
        self.create_event_type(
            &repo,
            "tms:execution:driver:checked-in",
            "Driver Checked In",
        ...
        self.create_event_type(
            &repo,
            "wms:inventory:receipt:completed",
            "Receipt Completed",
        ...
        self.create_event_type(
            &repo,
            "wms:inventory:putaway:completed",
            "Putaway Completed",
        ...
        self.create_event_type(
            &repo,
            "wms:inventory:cycle-count:completed",
        ...
        self.create_event_type(
            &repo,
            "wms:inventory:transfer:completed",
            "Transfer Completed",
        ...
        self.create_event_type(
            &repo,
            "oms:fulfillment:allocation:completed",
        ...
        self.create_event_type(
            &repo,
            "oms:fulfillment:backorder:created",
            "Backorder Created",
        ...
        self.create_event_type(
            &repo,
            "oms:fulfillment:split:occurred",
            "Order Split",
        ...
        self.create_event_type(
            &repo,
            "track:visibility:checkpoint:recorded",
            "Checkpoint Recorded",
        ...
        self.create_event_type(
            &repo,
            "track:visibility:delay:detected",
            "Delay Detected",
        ...
        self.create_event_type(
            &repo,
            "track:visibility:geofence:entered",
            "Geofence Entered",
        ...
        self.create_event_type(
            &repo,
            "track:visibility:geofence:exited",
            "Geofence Exited",
        ...
        self.create_event_type(
            &repo,
            "track:alerts:temperature:breach",
            "Temperature Breach",
        ...
        self.create_event_type(
            &repo,
            "yard:dock:appointment:scheduled",
        ...
        self.create_event_type(
            ...
            "Event Type Schema Finalised",
            "A schema version has been finalised as current",
        )
        .await?;
