...
impl RoleDto {
    fn from_role(role: AuthRole) -> Self {
        // Extract short name from full code (e.g., "myapp:admin" -> "admin")
        let name = role.code.split(':').nth(1)
            .unwrap_or(&role.code)
            .to_string();
...
pub async fn sync_roles(
    ...
    for role_req in &req.roles {
        ...
        if let Some(mut existing) = state.role_repo.find_by_code(&role_code).await? {
    ...
    if query.remove_unlisted {
        for existing in existing_sdk_roles {
            if !synced_codes.contains(&existing.code) {
...
pub async fn delete_role(
    ...
    // Get the role
    let role = state.role_repo.find_by_code(&role_code).await?
