...
impl<U: UnitOfWork> UpdateClientUseCase<U> {
    ...
    pub async fn execute(
        ...
        if let Some(ref desc) = command.description {
            if desc.len() > 500 {
                return UseCaseResult::failure(UseCaseError::validation(
                    ...
                    "Client description must be at most 500 characters",
                ));
            }
        }
        ...
        let mut updated_desc: Option<&str> = None;
        ...
        if let Some(ref desc) = command.description {
            let changed = client.description.as_deref() != Some(desc.as_str());
            if changed {
                client.description = Some(desc.clone());
                updated_desc = Some(desc.as_str());
            }
        }

        // Check if anything actually changed
        if updated_name.is_none() && updated_desc.is_none() {
            return UseCaseResult::failure(UseCaseError::validation(
                "NO_CHANGES",
                "No changes detected",
            ));
