...
| UoW owns tx? | **No (default).** `commit(event, cmd, persist?)` runs `persist()` then writes the event — caller owns atomicity. *Opt-in* `OutboxUnitOfWork.run(cb)` + `OutboxDriver.withTransaction?` DOES own a tx (on the class, not the interface). | **No.** `commit(event, cmd, persist?)` runs `persist()` then writes — never opens a tx. Only enforcement is `DatabaseDriver`'s `strictTransactions` flag (default OFF → warn-and-proceed). |
