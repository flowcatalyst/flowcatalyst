...
Status: DECIDED 2026-07-22 (owner) · Phase 1 implemented · Phases 2/2.5-v1
SUPERSEDED by Phase 2.5 v2 (separate portal identity plane, BUILT 2026-08-14)
Builder-facing guide: docs/portal-implementation-guide.md
...
> **Reading order.** Sections up to "Phase 2" are the decision history: the
> B2B2B split (platform authenticates, portal apps authorize) still stands,
> but the *mechanism* changed. Portal end-users are NO LONGER inert
> `iam_principals` rows and `POST /api/principals/portal` NO LONGER EXISTS —
> Phase 2.5 v2 (bottom of this doc) replaced them with a separate
> `portal_identities` population, `/portal/*` entry points, and
> `/api/portal-users`. Historical sections are kept for the rationale, not
...
- ~~Portal end-users exist in the platform as **inert identities**~~ —
  SUPERSEDED by Phase 2.5 v2: portal end-users are `portal_identities` rows,
  a population wholly separate from `iam_principals`. The inert-principal
  posture was built and then rejected (its safety depended on every future
  authorization surface honouring "null-client = inert" forever).
- ~~One human = one principal~~ — SUPERSEDED: identities are per
  (client, email) portal context. The same human may hold several portal
  identities and a platform account, each with independent credentials.
...
4. ~~**(Phase 2) Ensure/invite API** — `POST /api/principals/portal`~~ —
   SUPERSEDED: that endpoint was built and later REMOVED with the v2 pivot.
   Its successor is `POST /api/portal-users` (see Phase 2.5 v2), which
   creates portal identities instead of null-client principals.
...
- `organizations` (owner client implied by the portal deployment),
  `org_memberships` (portal identity id ↔ org, portal role, source
  MANUAL|INVITE|IDP_SYNC) — ordinary app tables in the portal DB. (v2: the
  join key is the id_token `sub`, a `ptu_…` portal identity id.)
...
- Login flow (v2): portal starts OAuth through `GET /portal/authorize` (one
  portal-flagged OAuth client per portal, bound to the owner client); the
  platform's portal login page routes password vs org-IdP by email domain;
  on callback the portal reads the id_token, resolves the user's
  membership(s), refuses users with no membership, and creates its own
  session. Multi-org users get an app-side org switcher (later phase).
- Design portal tables with promotion in mind (stable portal identity ids
  as the join key, org rows carrying an external identifier) so a future
  platform org model can be seeded from them.
...
## Phase 2 (SUPERSEDED — see Phase 2.5 v2)

- ~~`POST /api/principals/portal` ensure/invite endpoint~~ — built
  2026-08-11, removed 2026-08-13 with the v2 pivot; replaced by
  `/api/portal-users`.
