...
| 9 | `POST /api/principals/users` (`CreateUserRequest`: req `email,name`; opt `clientId,enforcePasswordComplexity,password,scope,sendInvitation,returnInviteLink`) | `POST /api/principals` | alias `POST /api/principals/users` → create handler (confirm create body == `CreateUserRequest`) | `principal/api/api.go:57` | T/M |
