...
| `FC_ROUTER_DEFERRAL_BUDGET` | `0` (router package default `5000`) | — | `internal/server/envcfg.go` | How many deferred messages one queue's consumer may have outstanding before it stops polling into pools that are all full. Sized against SQS FIFO's 20,000 in-flight ceiling, which deferred messages count toward. |
