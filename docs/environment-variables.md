...
| `FC_JWT_ACCESS_TOKEN_TTL_SECS` | `3600` | `OIDC_ACCESS_TOKEN_TTL` | `internal/server/envcfg.go` | Access-token lifetime in seconds. Sets both the minted `exp` and the `expires_in` every token response advertises, so the two can't disagree. The OIDC id_token has its own fixed 5-minute lifetime and is unaffected. |
| `OIDC_SESSION_TTL` | `86400` (24 h) | — | `internal/server/envcfg.go` | Session cookie + session JWT lifetime in seconds (password, passkey, 2FA and OIDC-bridge logins). The ECS task definitions set `28800` (8 h). |
| `OIDC_REFRESH_TOKEN_TTL` | `604800` (7 d) | — | `internal/server/envcfg.go` | Refresh-token lifetime in seconds. It is the family's **absolute** cap: rotation carries the original deadline forward, so a family dies this long after first issuance. The ECS task definitions set `2592000` (30 d). |
...
| `FC_ROUTER_CONFIG_INTERVAL_SECONDS` | `0` (router package default `300`) | `FLOWCATALYST_CONFIG_INTERVAL` | `internal/server/envcfg.go` | How often the router re-fetches `FLOWCATALYST_CONFIG_URL` while leader. |
| `FC_NOTIFY_BATCH_INTERVAL_SECONDS` | `0` (router package default `300`) | `NOTIFICATION_BATCH_INTERVAL` | `internal/server/envcfg.go` | How long the router's warning notifier holds non-critical warnings before flushing a batch to `FC_NOTIFY_WEBHOOK_URL`. A full batch (20) or a CRITICAL warning flushes immediately. |
...
| `FC_ROUTER_DEFERRAL_MAX_DELAY_SECONDS` | `0` (router package default `3600`) | — | `internal/server/envcfg.go` | Reservation horizon for messages a full pool hands back to the broker (`docs/router-architecture.md` §3a): the longest any one deferral asks the broker to hold a message, and so the longest a pool can idle waiting for already-deferred messages after its capacity improves. |
| `FC_ROUTER_DEFERRAL_BUDGET` | `0` (router package default `15000`) | — | `internal/server/envcfg.go` | How many deferred messages one queue's consumer may have outstanding before it stops polling into pools that are all full. Must be at least the largest slow-pool backlog the queue is expected to carry: SQS FIFO serves oldest-first, so reaching the other pools' messages means deferring the whole backlog ahead of them. Bounded by SQS FIFO's 20,000 in-flight ceiling, which deferred messages count toward; a backlog beyond ~15k needs its own queue. |
...
| `FLOWCATALYST_CONFIG_URL` | — | — | `internal/server/envcfg.go` | Router pool/queue configuration endpoint(s), comma-separated — the platform serves its own at `GET /api/dispatch/router-config` on the **API** listener, authenticated; fcdev defaults this to its own. Unset → the router starts with **no queues and no pools**. (`FC_DEFAULT_BROKER` used to synthesise a single fixed queue here; that branch is gone — dev and prod both learn their queues from a served document.) |
| `FC_ROUTER_CLIENT_ID` | `""` | — | `internal/server/envcfg.go` | OAuth client id the router authenticates to **its own** platform with, to fetch the router-config document. Must be set together with `FC_ROUTER_CLIENT_SECRET` and alongside `FC_ROUTER_PLATFORM_URL` — a half-configured credential is refused at startup. Unset → every config URL is fetched unauthenticated, and a platform-served URL then answers 401. |
| `FC_ROUTER_CLIENT_SECRET` | `""` | — | `internal/server/envcfg.go` | The matching client secret. The credential is sent **only** to URLs whose origin matches `FC_ROUTER_PLATFORM_URL`, so a comma-separated config URL listing third-party config services never receives it. |
