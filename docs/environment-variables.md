...
| `FC_JWT_ACCESS_TOKEN_TTL_SECS` | `3600` | — | `internal/server/wire_services.go` | Access-token lifetime in seconds. Sets both the minted `exp` and the `expires_in` every token response advertises, so the two can't disagree. The OIDC id_token has its own fixed 5-minute lifetime and is unaffected. |
