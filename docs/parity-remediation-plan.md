...
### ▶️ NEXT: Outbox processor (`internal/outbox/` + `{mongo,postgres,sqlite}` vs `crates/fc-outbox/`)
The original Phase 8 + #16 marked outbox "complete" (Mongo backend, crash recovery, leader-gating, per-item `{results}` parsing, max-retries, OB4 block-on-error, OB7 group-semaphore, operational state machine). **The deep per-subsystem audit has NOT yet been run on it** — do that next, same approach as router. Verify behaviourally vs `crates/fc-outbox/src/`: the claim/lease (find+update_many), `RecoverStuck` crash recovery, leader-gating (single-active for the non-atomic Mongo claim), the per-item `{results}` response parsing in `http_dispatcher.go` (already confirmed it expects `{items}`→`{results}` — and the platform now emits that after `89aad15`/`f534a53`, so the events path is end-to-end consistent), max-retries cap, OB4 group block-on-error + ordering, OB7 max-concurrent-groups semaphore, the per-group state machine (Running/Paused/Blocked + admin API on `FC_OUTBOX_ADMIN_PORT`), Mongo vs Postgres backend selection (`FC_OUTBOX_BACKEND`), and env aliases. Suggested fan-out: (a) dispatcher + response handling + retry/max-retries, (b) claim/recovery/leader-gating per backend, (c) group ordering/block-on-error/semaphore + state machine + admin API. Flag divergences + Go improvements as before.
