...
### ▶️ NEXT: Stream processor (`internal/stream/` vs `crates/fc-stream/` or equivalent)
Behavioural subsystems still un-deep-audited: **stream processor** and **scheduler** (both Phase-complete but not yet given the per-subsystem drop-in pass). Do the stream processor next, same approach: verify the event/dispatch projections (partition-key `created_at` preservation, `is_terminal`/`is_completed` semantics — Phase 7 ST1/ST2), the partition manager (leader-gating, expired-partition drop + retention, `is_partitioned` guard — ST3/ST4), and the per-projection batch/tuning env knobs (ST5) 1:1 vs the Rust stream source. Then scheduler (`internal/scheduler` dispatch-job + `internal/scheduledjob` cron engine vs Rust). Method reminder: Rust source 1:1 for behaviour; OpenAPI lock for any HTTP shapes. Flag divergences + Go improvements as before; verify agent findings against source (they over-claim).
