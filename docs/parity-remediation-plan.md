...
- **Recommended next:** all 10 phases + the high-value follow-ups are done & verified. **Remaining (all low-priority):** **#16** (outbox OB4/OB7 throughput batching — large, correct-today refinement), the **DB-secret rotation poller** (Phase 10; startup resolution works, only rotation deferred), and **O3** (`?provider=` niche feature — deferred, clean error today). #13-O7, #15-A2/A5/A4b-safety, the principal change-restriction (moot), Phase 9 `/api/me/applications`, and FU4 docker+MCP verification are all CLOSED. No net-new phases.
