...
## Deep-audit follow-up — per-subsystem drop-in verification (2026-06-01)

After the Phase 0–10 remediation below was declared complete, we ran **deeper, per-subsystem drop-in audits** (source-to-source vs the Rust reference, fanned out with read-only agents then verified by hand). These found and fixed real gaps the phase work missed. **All work below is committed AND pushed to `origin/main`.** Method reminder: behavioural subsystems (router/outbox/stream/scheduler) are verified against Rust **source** 1:1; platform HTTP shapes via the OpenAPI lock + source. Agent findings were verified against source before acting — several over-claimed (see memories).

### ✅ Platform layer (`internal/platform/` vs `crates/fc-platform/src/`) — DONE
- **Real drop-in break fixed:** batch-ingest endpoints returned `{accepted:N}` but the outbox dispatcher requires `{results:[{id,status,error?}]}` → events never acknowledged. Unified contract on events + dispatch-jobs: request `{items}`, response `{results}`, cap 1000; Go events item now accepts snake_case SDK aliases + preserves `messageGroup`/`specVersion`. Rust dispatch-jobs SDK endpoint switched `{jobs}`/`{jobs,count}` → `{items}`/`{results}`. Commits: Go `89aad15`, Rust `f534a53`.
- Added the missing `POST /api/audit-logs/batch` endpoint (Go audit repo was read-only) — `bcd54cf`.
- Wire alignments (in `89aad15`): `POST /api/principals` anchor-only caller (any scope creatable); IAM/client mutations return the updated entity / `{message}` not `204`; `event-types/{id}/versions` returns `EventTypeResponse`; CORS `/allowed` field `allowedOrigins`→`origins` + `GET /api/platform/cors/{id}`; oauth-clients activate/deactivate `{success,message}`; built `/api/me/clients[...]` + `/auth/client/{accessible,switch,current}`.
- IAM read-model divergence de-risked via DATA (owner chose this over a code change): optional operator script `tools/backfill-iam-read-perms.sql` grants `platform:iam:user:view` to user-created roles (`1205957`). Plus a `principal.list` filter fix to hide `client_id IS NULL` principals from non-anchors (`2dcdfff`). See memory [[batch-ingest-and-wire-alignment]].

### ✅ Message router (`internal/router/` vs `crates/fc-router/src/`) — DONE (`1186b24`)
No correctness bugs; HMAC signing byte-identical; routing/dispatch/retry/breaker/config-sync/standby/ALB all matched. Fixed the behavioural divergences (Go improvements left intact): StallDetector force-NACK implemented (was dead config); consumer auto-restart hardened (attempt tracking + 5s delay + `CONSUMER_HEALTH` warning → CRITICAL after 10 + clear-on-recover); stall threshold 90s→60s; added `QUEUE_HEALTH`+`CONSUMER_HEALTH` categories; Notifier critical-flush + `MinSeverity`. See memory [[router-parity-status]] (incl. gotchas: Rust `record_message_processed` is dead code; native Teams/SMTP notification is an intentional Go gap).

### ✅ Outbox processor (`internal/outbox/` + `{mongo,postgres}` vs `crates/fc-outbox/`) — DONE (2026-06-01)
Deep per-subsystem audit run (3-way read-only fan-out: dispatcher/retry, claim/recovery/leader, groups/state/admin — then verified by hand vs Rust source). **No drop-in / wire-shape / shared-schema break found**: request body `{items:[rawPayload]}`, URL/method/`Authorization: Bearer`/content-type, per-item `{results:[{id,status,error}]}` parsing, the SCREAMING_SNAKE_CASE status vocabulary, the int status codes (0/1/2/3/4/5/6/9), retryable/terminal sets, DDL + partial indexes, Mongo doc shape + indexes, delete-on-success, stuck-definition/threshold(300s)/interval(60s), and the max-retries boundary (both cap at 3 total dispatch attempts, no off-by-one) all **MATCH** Rust 1:1.

**One real behavioural divergence fixed** (`http_dispatcher.go`): Go bucketed *all* 4xx → non-retryable `BAD_REQUEST`, so a transient **429** (or 404/409/422) **permanently failed and BLOCKED its message group**; Rust maps only an exact `400→BAD_REQUEST` and everything else (other 4xx, 501/505, etc.) → `INTERNAL_ERROR` (retryable). Now byte-identical to `http_dispatcher.rs`'s match arms. Same edit aligned the **transport-error** status to `GATEWAY_ERROR` (was `INTERNAL_ERROR`; both retryable, but Rust persists `GATEWAY_ERROR` into the shared table). Added regression tests (`TestSend_400IsTerminalBadRequest`, `TestSend_Transient4xxIsRetryable`, `TestSend_TransportErrorIsGatewayError`, `TestSendBatch_Transient4xxIsRetryable`). **Drop-in env alias** (`envcfg.go`): `FC_OUTBOX_BACKEND` now falls back to Rust's var name `FC_OUTBOX_DB_TYPE` (`FC_OUTBOX_DB_URL` was already honoured as the Mongo-URI alias). See memory [[outbox-parity-status]].

**Go improvements left intact (deliberate, not divergences):** atomic Postgres claim via `FOR UPDATE SKIP LOCKED` CTE (Rust's SELECT+UPDATE is non-atomic, leans on the leader gate); a *real* `FC_OUTBOX_MAX_CONCURRENT_GROUPS` semaphore (Rust's `max_concurrent_groups` is misnamed — it's `×10`'d into a live-actor *capacity*, not a dispatch cap); explicit `INTERNAL_ERROR` retry for an item missing from `{results}` (Rust `zip`-truncates and silently drops it to recovery); unknown per-item status scoped to one item (Rust serde-fails the whole batch parse); no per-pass recovery `LIMIT`.

**Deferred / known divergences (low-value, documented — not fixed):** (1) **per-type split tables** — Rust honours `FC_OUTBOX_{EVENTS,DISPATCH_JOBS,AUDIT_LOGS}_TABLE` (all default `outbox_messages`); Go hard-codes the single shared `outbox_messages` and never filters by `type` (id-only). Safe for the SDK default (single table per `001_create_outbox_messages.sql`); only a break if an operator split tables — build on demand, not blind. (2) **Postgres outbox reuses the shared pgxpool** (owner decision in subsystems.go) rather than a separate `FC_OUTBOX_DB_URL` connection like Rust — intentional single-binary model. (3) 10s connect-timeout not set (Go has the 30s total only); retry cadence is poll-gated (~1s) vs Rust's in-memory front-of-queue tight loop — internal timing only, no contract impact.

### ✅ Stream processor (`internal/stream/` vs `crates/fc-stream/`) — DONE (2026-06-01)
Deep per-subsystem audit (3-way read-only fan-out: projections, fan-out, partition-manager/health/config — then hand-verified vs Rust source). **No shared-schema / read-model contract break.** Read-model tables/columns (`msg_events_read`, `msg_dispatch_jobs_read`), ST1 source-`created_at` preservation, the event field mapping, `ON CONFLICT (id,created_at)` upsert keys, the subscription matcher (`:`-segment wildcard + tri-state client scoping), fan-out cardinality/idempotency-key/`message_group` propagation, dispatch-job columns, partition create/drop (month-window math, `pg_inherits` discovery, name→date parse, `is_partitioned` guard — no off-by-one, no data-loss on a non-partitioned drop-in), and health aggregation all MATCH Rust 1:1.

**Behavioural divergences fixed** (`projector.go`/`subsystems.go`/`envcfg.go`/`partition_manager.go`): (1) the projector loop now mirrors Rust's `adaptive_sleep` — a **full batch loops with 0ms** (drains a backlog at full speed; Go previously paused 100ms between every batch), partial→100ms, idle→1s, and **error back-off 1s→5s** (`nextSleep` extracted + unit-tested). (2) **Batch-size defaults aligned to Rust** (events/dispatch 100, fan-out 200; Go defaulted all three to 500 while the comment falsely claimed parity) — precedence now per-projection env > global `FC_STREAM_BATCH_SIZE` > Rust default. (3) **`FC_STREAM_FAN_OUT_SUBS_REFRESH_SECS`** is now read + wired into the fan-out subscription-cache TTL (Rust knob; Go hardcoded the matching 5s default and ignored the env). (4) partition-manager health `processed_count` now counts created+dropped (was always 0). See memory [[stream-parity-status]].

**Go improvements left intact (intentional, not divergences):** projections + fan-out run **un-gated, multi-replica via `FOR UPDATE SKIP LOCKED`** (only the partition-manager DDL is leader-gated) — Rust fc-server instead gates the *whole* stream processor on the leader, so non-leaders project nothing; the SKIP-LOCKED model (= Phase 7 ST3 decision, also matching Rust's *standalone* binaries) gives better projection throughput and is safe. ST2 `is_completed = status=COMPLETED` only / `is_terminal = COMPLETED|FAILED|CANCELLED|EXPIRED` matches the canonical `DispatchStatus` (`fc-common`); the Rust **projection file** is the buggy side here (references never-emitted `SUCCESS`/`IGNORED` and inverts the two flags) — Go is correct, do not "align" to it.

**Verified NON-issues (agents over-claimed — confirmed against source, no change):** (a) **`connection_id`** — Rust's fan-out INSERTs it but **no Rust migration ever adds it to `msg_dispatch_jobs`** (004 lacks it; 009 only touches `msg_subscriptions`; 011 is a no-op `IF NOT EXISTS`; 012 adds `projected_at`), nothing in the dispatch path reads `job.connection_id`, and **both** sides set the real delivery target via `target_url ← sub.target`. Canonical schema has no such column; Go correctly omits it (writing it would fail). (b) **dispatch-job `id`** — both schemas declare `id VARCHAR(13)`; Rust emits `djb_`+13 = 17 chars (overflows), Go uses 13-char untyped (fits) — Go is the correct drop-in. (c) **per-subscription filtering** — neither side applies it; pre-existing shared functional gap, no Go↔Rust divergence.

### ▶️ NEXT: Scheduler (`internal/scheduler` dispatch-job + `internal/scheduledjob` cron vs Rust)
Last behavioural subsystem un-deep-audited. Two engines: the **dispatch-job scheduler** (`internal/scheduler`, Phase 6 SC-series — claims QUEUED dispatch jobs, the SKIP-LOCKED un-gated path) and the **scheduled-job cron engine** (`internal/scheduledjob`, Phase 6 — poller `LatestSlotInWindow`/`MarkFired` monotonic + dispatcher QUEUED→IN_FLIGHT→202 contract, leader-gated via its own election). Verify 1:1 vs the Rust scheduler source: the cron firing parser (6-field seconds-first / cron-crate parity; 5- and 7-field never fire), skip-to-latest window math, the WebhookEnvelope POST→202 delivery contract + requeue at `delivery_max_attempts`, FireNow two-phase MANUAL insert, `hasActiveInstance`/Concurrent gating, and the leader-election co-location nuance (Go uses a subsystem-suffixed lock key vs Rust's shared lease). Check the Phase 6 follow-up nits in this doc's Resume notes. Same method; verify agent findings against source (they over-claim — this audit caught 3 false alarms).
